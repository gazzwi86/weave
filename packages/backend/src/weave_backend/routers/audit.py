...
from weave_backend.audit.listing import AuditFilters, count_by_event_type, list_entries
...
def _filters_from_params(params: AuditQueryParams | AuditCountsQueryParams) -> AuditFilters:
    return AuditFilters(
...
@router.get("/counts", response_model=EventTypeCountsResponse)
async def count_audit_entries_by_event_type_route(
    params: Annotated[AuditCountsQueryParams, Query()],
    principal: Annotated[Principal, Depends(require_tenant_admin)],
) -> EventTypeCountsResponse:
    """G6: powers dashboard cards D/E/F/I -- same tenant-admin gate and
    filter set as `GET /api/audit`, grouped by event_type."""
    _require_own_tenant(principal, params.tenant_id)
    async with tenant_connection(principal.tenant_id) as conn:
        counts = await count_by_event_type(
            conn, tenant_id=principal.tenant_id, filters=_filters_from_params(params)
        )
    return EventTypeCountsResponse(
        counts=[EventTypeCountResponse(event_type=c.event_type, count=c.count) for c in counts]
    )
