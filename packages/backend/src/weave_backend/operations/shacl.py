...
def _to_result(report: Graph, result_node: Node, shapes: Graph, data_graph: Graph) -> ShaclResult:
...
def validate_graph(data_graph: Graph) -> list[ShaclResult]:
    ...
    return [_to_result(results_graph, node, shapes, data_graph) for node in result_nodes]
