...
import type {
  NeighbourElement,
  RendererAdapter,
} from "@/lib/explorer/renderer-adapter";
...
describe("useNeighbourExpansion", () => {
  ...
  it("expands immediately when the new-node count is at or under the threshold", () => {
    ...
    const { result } = renderHook(() =>
      useNeighbourExpansion({ adapter, config }),
    );

    act(() =>
      result.current.requestExpand("n1", [neighbour("a"), neighbour("b")]),
    );
  ...
  it("excludes already-present nodes from the new-node count", () => {
    const adapter = fakeAdapter({
      listNodes: vi.fn(() => [
        { id: "a", label: "a", typeLabel: "t", bpmoKind: "DataAsset" },
      ]),
    });
    const config = { ...DEFAULT_EXPLORER_CONFIG, expandConfirmThreshold: 1 };
    const { result } = renderHook(() =>
  ...
  it("collapses a node directly", () => {
    const adapter = fakeAdapter();
    const { result } = renderHook(() =>
      useNeighbourExpansion({ adapter, config: DEFAULT_EXPLORER_CONFIG }),
    );
