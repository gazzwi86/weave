...
/** TASK-013: the chat panel polls this to learn when an ingest job has
 * finished extracting and moved to `awaiting-review`.
 */
export async function GET(
  ...
  try {
    upstream = await fetch(`${backendUrl}/api/ingest/jobs/${encodeURIComponent(parsed.data)}`, {
