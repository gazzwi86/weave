...
/** BE-V1-TASK-018 AC-3: proxies `GET .../tasks/{task_id}/captures` -- the
 * 8-state visual capture manifest, `{"manifest": null}` on honest absence
...
export async function GET(_request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  return forwardToBackend(
    `/api/projects/${encodeURIComponent(id)}/tasks/${encodeURIComponent(taskId)}/captures`,
    caller.token
  );
}
