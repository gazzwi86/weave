...
/** H4: reads the CE-G12 pending-gates feed (weave_backend's
 * `get_pending_gates_route`) for the dashboard "Needs you" gates row --
 * no role guard, same read-any-tenant-member posture as pin-diff/decisions. */
export async function GET(_request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  return forwardToBackend(`/api/projects/${encodeURIComponent(id)}/gates?status=pending`, caller.token);
}
