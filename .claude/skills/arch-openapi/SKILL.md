...
4. `.claude/specs/<entity>/04-arch/tech-spec.md` (if present) — resource list, auth
   approach, non-functional requirements
5. `.claude/specs/<entity>/02-prd/prd.md` (if present) — user stories that drive endpoint
   shape
6. Any existing `openapi.yaml` draft at `.claude/specs/<entity>/04-arch/tech-spec/openapi.yaml`
...
```bash
python3 -c "import yaml, sys; yaml.safe_load(open('.claude/specs/<entity>/04-arch/tech-spec/openapi.yaml'))" \
...
```bash
git add .claude/specs/<entity>/04-arch/tech-spec/openapi.yaml
...
File: `.claude/specs/<entity>/04-arch/tech-spec/openapi.yaml`
