...
1. `CLAUDE.md` — Weave product context, confirmed stack, Plugin Laws A-F
2. `.claude/spec-templates/epic.md` — section structure (use as scaffold, never leave `{{}}` in output)
3. `docs/specs/<entity>/02-prd/prd.md` — the parent PRD; locate the epic section that this file
   will cover (section heading, priority, linked stories)
4. `docs/specs/<entity>/02-prd/epics/` — scan existing EPIC-NNN.md files to determine the next
...
`docs/specs/<entity>/02-prd/epics/EPIC-NNN.md`
...
- Scan `docs/specs/<entity>/02-prd/epics/` for existing tasks to avoid collisions
...
0. Append a `# Related` section linking predecessor documents with path-relative markdown
   ...
   written. Do not forward-link task briefs (`../../04-arch/tasks/*.md`) until they exist.
...
**File:** `docs/specs/<entity>/02-prd/epics/EPIC-NNN.md`
...
**Template:** `.claude/spec-templates/epic.md`

Create the directory if it doesn't exist:

```bash
mkdir -p docs/specs/<entity>/02-prd/epics/
```
...
resource: docs/specs/<entity>/02-prd/epics/EPIC-NNN.md
---
```
