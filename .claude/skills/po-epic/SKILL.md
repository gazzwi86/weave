...
3. `.claude/specs/<entity>/02-prd/prd.md` — the parent PRD; locate the epic section that this file
   will cover (section heading, priority, linked stories)
4. `.claude/specs/<entity>/02-prd/epics/` — scan existing EPIC-NNN.md files to determine the next
...
`.claude/specs/<entity>/02-prd/epics/EPIC-NNN.md`
...
- Scan `.claude/specs/<entity>/02-prd/epics/` for existing tasks to avoid collisions
...
**File:** `.claude/specs/<entity>/02-prd/epics/EPIC-NNN.md`
...
```bash
mkdir -p .claude/specs/<entity>/02-prd/epics/
```
