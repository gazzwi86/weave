...
### 4.7 Strands/AgentCore/Bedrock division (DECIDED — broad strokes)

- **Strands Agents SDK** = authoring layer for AI agents generated by Weave's Build Engine.
...
> **Rule:** Generate portable Strands code (the "IR" of the Build Engine). Deploy to
...
- **Strands/AgentCore conventions** — when generated code uses Strands SDK, verify
  pattern conformance against `spec-templates/few-shot/` Strands examples
...
|---|---|---|
| Backend language | Python 3.12+ | Strands SDK primary; data science ecosystem |
...
| Dimension | Default | Rationale |
|---|---|---|
| Agent authoring | AWS Strands SDK (Python + TS 1.0) | Open-source, MCP + A2A, runs on Anthropic direct or Bedrock |
