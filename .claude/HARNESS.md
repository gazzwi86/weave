...
| run-loop.sh | script | Deterministic driver for limit-spanning /implement runs: high-tier→mid-tier fallback, sleep-on-limit, halts at every HITL gate (ADR-H1 reopen) | Operator: bash .claude/scripts/run-loop.sh | Long runs die at the first usage limit and need manual restarts | active | TODO |
