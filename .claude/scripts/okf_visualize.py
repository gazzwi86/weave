...
Run:  uv run .claude/scripts/okf_visualize.py docs [-o docs/viz.html] [--exclude docs/wiki/viz.html]

Source: adapted from https://github.com/scaccogatto/okf-skills (MIT licence)
"""
...
def _excluded(p: Path, excludes: list[Path]) -> bool:
    rp = p.resolve()
    for ex in excludes:
...
def build(bundle: Path, excludes: list[Path] | None = None):
    excludes = [e.resolve() for e in (excludes or [])]
    nodes, edges, seen = [], [], set()
    files = sorted(
        p for p in bundle.rglob("*.md")
        if p.is_file() and p.name not in RESERVED and not _excluded(p, excludes)
    )
...
def render(bundle: Path, out: Path, title: str | None = None, layout: str = "cose",
           excludes: list[Path] | None = None):
    nodes, edges = build(bundle, excludes)
    name = title or f"{bundle.resolve().parent.name}/{bundle.name}"
    html = (HTML.replace("__NAME__", name).replace("__LAYOUT__", layout)
...
def main() -> int:
    ...
    ap.add_argument("--exclude", action="append", type=Path, default=[],
                    help="path (file or dir) to exclude from the graph; repeatable")
    args = ap.parse_args()
    if not args.bundle.is_dir():
        print(f"error: {args.bundle} is not a directory", file=sys.stderr)
        return 2
    out = args.out or (args.bundle / "viz.html")
    n, e = render(args.bundle, out, title=args.title, layout=args.layout,
