...
tags: [build-engine, 04-arch, tech-spec, m1, business-process]
status: Draft
timestamp: 2026-07-01T00:00:00Z
resource: docs/specs/weave/engines/build-engine/04-arch/tech-spec/business-process.md
...
flowchart TD
  ...
  C -->|draft_spec_only| D[AI drafts spec — claude-opus-4-8]
  C -->|spec_to_build| D
...
sequenceDiagram
  ...
  participant Arch as Architect Agent<br/>(claude-opus-4-8)
  participant Val as Brief Validator<br/>(claude-haiku-4-5)
...
sequenceDiagram
  autonumber
  participant Arch as Architect Agent<br/>(claude-opus-4-8)
  participant Eng as Task Engineer<br/>(claude-sonnet-4-6)
  participant Haiku as CODIFY Validator<br/>(claude-haiku-4-5)
...
flowchart LR
  ...
  B --> C[Generate Next.js UI<br/>claude-sonnet-4-6]
  B --> D[Generate FastAPI API<br/>claude-sonnet-4-6]
...
async def m1_generate_loop(job: BuildJob) -> None:
    ...
    artefacts   = await generate_nextjs_fastapi(graph_data, job)   # claude-sonnet-4-6
...
sequenceDiagram
  ...
  participant CE as Constitution Engine
  participant Gen as Code Generator<br/>(claude-sonnet-4-6)
...
and security spike ([CE TASK-008](../../../constitution-engine/04-arch/tasks/TASK-008.md)) stress-
