...
resource: docs/specs/weave/engines/build-engine/m1/tasks/TASK-003.md
---
...
> `CE-READ-1`, streamed, 60 s timeout, `claude-fable-5`).
...
| AC-8 | WHEN the LLM is invoked, THE SYSTEM SHALL use only `claude-fable-5` for spec drafting; a routing miss (model unavailable) MUST halt the pipeline and return `503 {"error": "model_unavailable"}` | `test_drafting_uses_claude_fable_model_id` |
...
async function run_drafting_pipeline(request_id, tenant_id, prompt, run_mode):
  ...
  # Stream generation via claude-fable-5
  ...
  with timeout(SPEC_DRAFT_TIMEOUT_S):         # default 60, from PLAT-SETTINGS-1
    for section in sections:
      stream = anthropic_client.messages.stream(
        model="claude-fable-5",              # hard-coded; routing miss → 503
...
| LLM: `claude-fable-5` for spec drafting | [CLAUDE.md](../../../../../../../CLAUDE.md#stack-confirmed) | Hard-coded model ID in routing table; routing miss → 503, never silent invocation of unapproved model |
...
- **Estimated cost:** ~$0.90 (claude-fable-5 pricing at time of writing; verify in MEMORY.md)
