...
created: 2026-07-01
blocked_by: []
unlocks: [TASK-004, TASK-006, TASK-010]
adr_refs: []
...
resource: docs/specs/weave/engines/build-engine/m1/tasks/TASK-001.md
---
...
>
> **Source-control config (M1 producer for TASK-010).** This record also captures the project's
> **source-control provider setting + Secrets-Manager token reference** (config value only — no settings
> UI), because run step 0 (E11-S7 / TASK-010 repo bootstrap) resolves it at M1 to create the external
> repo. Accept an optional `source_control` object (`{provider: "github"|"gitlab", token_secret_ref}`)
> on create and persist it on the project record; the token value itself lives in AWS Secrets Manager,
> never on the record. The full settings UI (E2-S6) is v1.0.
...
- **Estimated cost:** ~$0.40 (claude-fable-5 pricing at time of writing; verify in MEMORY.md)
