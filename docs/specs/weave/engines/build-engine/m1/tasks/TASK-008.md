...
resource: docs/specs/weave/engines/build-engine/m1/tasks/TASK-008.md
---
...
>
> **Stack & output target.** The Build Engine is **stack-agnostic (decision B8)** — the generated
> stack is driven by the client company/project standards (FR-063/FR-064, M2). This M1 task uses
> **Next.js + FastAPI as the demo default** because the standards catalogue lands at M2. Generated code
> is **committed and pushed to the project's NEW external repo** (created at run start by the
> repo-bootstrap step, TASK-010 / FR-061), never to a repo inside Weave.
...
| AC-7 | WHEN generated configuration or code references Claude model IDs, THE SYSTEM SHALL use only the confirmed set (`claude-fable-5`, `claude-sonnet-5`); any placeholder or invented model ID MUST cause a SAST-pattern violation that fails the pipeline | `test_generated_code_uses_only_confirmed_model_ids` |
...
function generate_app(jwt, project_iri, task_id):
  ...
  # All gates passed — commit to a feature branch on the project's EXTERNAL repo
  # (created at run start by the repo-bootstrap step, TASK-010/FR-061; GitHub or GitLab per
  # the project's source-control setting). Output never lives inside Weave.
  branch = f"build/{project_iri.split(':')[-1]}/{task_id}"
  commit_sha = git_client.commit_workspace(project.repo, workspace, branch,
...
| Engineer agent model: `claude-sonnet-5` | [CLAUDE.md](../../../../../../../CLAUDE.md#stack-confirmed) | Hardcoded in routing table; generation uses Sonnet (speed/cost balance); Fable is advisor role only |
...
| Output pushed to the project's NEW external repo (GitHub/GitLab) | [build-engine.md B9 / FR-061](../../../build-engine.md#key-decisions) | Commit target is `project.repo` (created by TASK-010 at run start), never a repo inside Weave |
| Stack-agnostic generation; Next.js + FastAPI is the M1 demo default only | [build-engine.md B8 / FR-063/FR-064](../../../build-engine.md#key-decisions) | Do not treat Next.js+FastAPI as a constraint on output; from M2 the stack is chosen from client company/project standards |
...
- **blocked_by:** [TASK-006, TASK-010]
- **unlocks:** [TASK-009]
- **External prerequisites:** `"Bandit, Semgrep, mypy, tsc, mutmut available in agent execution environment"`, `"Project external repo created by TASK-010 (repo-bootstrap) and source-control token available via Secrets Manager"`, `"CE-READ-1 endpoint available"`

## Cost Estimate

- **Complexity:** L
- **Estimated tokens:** ~15k input, ~8k output
- **Estimated cost:** ~$1.10 (claude-fable-5 pricing at time of writing; verify in MEMORY.md)
