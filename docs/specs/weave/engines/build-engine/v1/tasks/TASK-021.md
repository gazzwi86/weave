---
type: Task
title: "Task: TASK-021 — Direct Project Prompt (FR-065): Role-Gated Prompt → Dark-Factory Run"
...
resource: docs/specs/weave/engines/build-engine/v1/tasks/TASK-021.md
---

# Task: TASK-021 — Direct Project Prompt (FR-065): Role-Gated Prompt → Dark-Factory Run
...
> verbatim** — turn caps, retry taxonomy, safety gates, HITL gates, budget checks (TASK-013),
> cost attribution (TASK-012), external-repo targets (FR-061). This task adds an entry point
...
no new status endpoint. Consumes: lifecycle enqueue (M1), Role Guard (TASK-011),
`project_prompts` (TASK-010), cost attribution rides TASK-012 (prompt-run dispatches carry
...
| Prompt box visible-but-disabled for readers | UX mirror rule (TASK-015 AC-4 posture) | Discoverability without implying permission; server 403 remains the boundary |
...
- `should apply caps and gates identically to prompt runs` (prompt run against a seeded
  breach fixture halts at checkpoint — reuses TASK-013's fixture)
...
- **blocked_by:** [TASK-012, TASK-019] (cost attribution live for prompt runs; the Dashboard
