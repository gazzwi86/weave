---
type: Task
title: "Task: TASK-019 — Project Dashboard (FR-013): Per-Tile Isolated Status View"
...
resource: docs/specs/weave/engines/build-engine/v1/tasks/TASK-019.md
---

# Task: TASK-019 — Project Dashboard (FR-013): Per-Tile Isolated Status View
...
> buttons beyond "Open Kanban" navigation. The prompt box on this page is TASK-021.
...
| AC-4 | WHEN budget/forecast tiles render, THE SYSTEM SHALL show TASK-013's labelled estimates, the binding cascade level, and the forecast inputs — never an unexplained number | `should render budget forecast with estimated label and inputs` |
...
GET /api/projects/{id}/dashboard/{tile}:      # one route, tile param enum — six handlers
    ...
    budget:   TASK-013 rollup total + binding cap + level
    forecast: TASK-013 forecast + inputs
    tasks:    spine counts by lane (reuse TASK-017 lane mapping)
...
would recouple tile failure modes). Consumes TASK-013 costs internals, spine reads, projects
...
- **blocked_by:** [TASK-013, TASK-014]
- **unlocks:** [TASK-021] (the prompt box mounts on this page)
