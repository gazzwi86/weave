---
type: Task
title: "Task: TASK-004 — Costs Endpoint + Budget-Cascade Breach Halt (ADR-008 read side, FR-008)"
...
resource: docs/specs/weave/engines/build-engine/v1/tasks/TASK-004.md
---

# Task: TASK-004 — Costs Endpoint + Budget-Cascade Breach Halt (ADR-008 read side, FR-008)
...
async def get_costs(project_id, ctx):
    rows = repo.cost_events.rollup(ctx.tenant_id, project_iri)        # TASK-001 aggregate
...
| Forecast formula = ADR-008 §4, pinned once in AC-2 | [ADR-008](../../decisions/ADR-008.md) #4 | One formula in one place (AC-2 = pseudocode = tile); the Dashboard tile never shows an unexplained number (TASK-010 renders these inputs) |
...
Deferred to TASK-010 (Dashboard budget/forecast tiles render this payload; the tile E2E
...
- **blocked_by:** [TASK-003] (cost_events rows exist)
- **unlocks:** [TASK-010]
