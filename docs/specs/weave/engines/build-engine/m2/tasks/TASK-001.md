...
description: "Implement the workspace standards catalogue (FR-063) and project-level overrides
  (FR-064) per ADR-007: standards_documents table, CRUD API, policy_iri validation via CE-READ-1,
  effective-set resolution consumed by E8-S1 prompt assembly."
...
resource: docs/specs/weave/engines/build-engine/m2/tasks/TASK-001.md
---
...
> **FRs covered:** FR-063 (company/workspace standards), FR-064 (project overrides,
> tighter-wins). API-only in M2 — the authoring UI is v1.0 (E2). Brand/voice stays CE-BRAND-1
> (TASK-002), NOT here.
...
| AC-4 | WHEN generation (E8-S1) runs for a project whose workspace has an empty catalogue, THE SYSTEM SHALL proceed with the M1 demo-default stack and write a `standards_missing` warning to the run log — absence degrades, never halts | `should degrade to demo-default stack with warning when catalogue empty` |
...
function put_standard(jwt, scope, key, body):
  ...
  row = repo.standards.upsert(tenant=claims.tenant_id, workspace=body.workspace_id,
...
function effective_set(tenant, workspace, project_id):
  company = repo.standards.list(tenant, workspace, scope='company', status='active')
  project = repo.standards.list(tenant, workspace, scope='project',
...
function build_generation_context(project):
  std = effective_set(project.tenant, project.workspace, project.id)
  if not std:
...
Request: `{workspace_id, project_id?, title, body_md, stack_pins?, policy_iri, status?}`
Response `200`: the persisted row. Errors:

| Status | Condition |
|---|---|
| 401 | Missing/invalid JWT |
| 403 | Principal lacks workspace-admin role |
| 404 | Workspace not found |
| 422 | scope/project_id mismatch, bad status, `policy_not_found` |
| 503 | CE-READ-1 unreachable (`ce_unavailable`) |
| 500 | Unexpected |

**`GET /api/standards?workspace_id=&scope=&project_id=`** — p95 ≤ 300 ms. `200` list; 401/403/500.
...
- **blocked_by:** []
- **unlocks:** [TASK-003] (both touch the E8-S1 generation-context builder; this task lands first)
- **External prerequisites:** CE-READ-1 entity lookup available (stubbed in tests); Alembic
  migration slot for `standards_documents` + `projects` columns (m2-delta §4)
...
- Workspace-admin authz: reuse the M1 role check used by project settings; no new role logic.
