...
task owns the audit-sourced activity feed with friendly actors.

- Widget renders as `ActivityFeed` — event-log data shape maps to it directly
  (`docs/standards/generative-ui.md` intent-mapping table: "event log → activity feed"), consumed
  from the Storybook design-system library once `PLAT-V1-TASK-026` (R13 Storybook foundation)
  lands (`docs/design/visual-direction.md` R13 delivery approach).
- Actor rendering: every row's `actor` field SHALL render as `EntityRef` (friendly label + mono ID
  chip) — never the raw `urn:weave:principal:...` string. This is both the R1 chrome primitive
  and R2's explicit "no raw principal URN anywhere on the surface" rule
  (`docs/design/v1-design-requirements.md` R1/R2; F-D08 finding,
  `design-assessment-2026-07-09.md` — the same raw-URN-as-hero failure this widget must not
  repeat).
- Timestamps (`ts` field) render via the relative-time component, not a raw ISO string — "relative
  time … used everywhere raw URNs/ISO timestamps appear" (R1 bundle; F-D08 finding names full ISO
  timestamps wrapping two lines in tables as the exact failure mode).
- Entity deep-link (AC-2, `/resource/{iri}`) SHALL render as `EntityRef` (friendly label + mono ID
  chip) as link text, never the raw IRI — F-D08 explicitly names "created-entity link text is the
  full IRI" as the pattern to avoid.
- Draft badge (`version_iri: null`, AC-2) is icon + text, never colour-alone
  (`docs/standards/design/components.md` badge/chip rule) — this is the token-level backing for
  this task's own DoD line "badges text + colour".
- Degradation states per `docs/standards/design/components.md` §Data-widget states: stale (AC-4)
  → `--color-text-muted` badge + `--font-mono` timestamp, content dimmed not hidden; the 410
  "history truncated" notice (AC-3) and the post-v1 "available post-v1" state (AC-5) both render
  as a named-reason tile, never blank (`docs/standards/generative-ui.md`: "no data source →
  defined unavailable state … never a blank or hallucinated widget").
- Loading: `shimmer` skeleton over `--color-raised` while the first page of rows streams in,
  reduced-motion → static `--color-raised` block (`docs/standards/design/components.md`
  §Data-widget states).
- Footer cites `CE-EVENT-1` + `CE-READ-1` (this task's own DoD requirement); token treatment is
  `--text-caption` `--color-text-muted` (`docs/standards/design/components.md` footer row).
- JTBD: the entity deep-link is what satisfies "one click reaches the change itself"
  (`docs/design/jtbd.md` Home/Dashboard success criterion).

**Advisory (uncited, not an acceptance criterion):** newly-appended rows from a polling refresh
are not named as an `aria-live` case in `components.md` (its `aria-live="polite"` treatment is
given for streamed/async toasts, not a polling activity feed) — recommend the same
`aria-live="polite"` announcement on the row container so screen-reader users notice new activity
without re-scanning the list; not a cited rule, flag for confirmation at sign-off.

**Advisory (uncited, not an acceptance criterion):** `docs/standards/design/layout-grid.md`
names column-span numbers for `KpiCard` (3) and the `LineChart`/`DataTable`/`AlertBanner` family
(6–12, full) but does not give `ActivityFeed` a specific span — pick a span in the 6–12 range
consistent with other list/table-shaped tiles; engineer's call, not a cited requirement.
