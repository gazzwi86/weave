...
task owns the KPI bento tiles (the six `DEFAULT_TILES` + starters); the "Needs you" list and
activity-feed requirements are attached to TASK-016 and TASK-024 respectively.

- Every KPI tile renders as a generative-ui catalogue component selected by data shape: scalar
  fields (`entity_count_by_kind` sum, `latest_version`, `draft_published_delta`,
  `owl_inconsistencies`) → `KpiCard`; categorical fields (`entity_count_by_kind` breakdown,
  `shacl_errors_by_severity`) → `BarChart` (`docs/standards/generative-ui.md` intent-mapping
  table).
- Tiles are consumed from the Storybook design-system library, not hand-rolled markup, once
  `PLAT-V1-TASK-026` (R13 Storybook foundation) lands — this task's own page composes data into
  the library's `KpiTile`/chart components and does not own bespoke CSS
  (`docs/design/visual-direction.md` R13 delivery-approach + atomic-design ruling).
- Grid placement: 12-column CSS Grid at `--bp-xl`+ collapsing 4 → 3 → 2 → 1 per breakpoint;
  `--space-6` gap between tiles, `--space-4` internal tile padding; `KpiCard` spans 3 columns,
  a `BarChart` tile spans wider per the catalogue's column-span rule
  (`docs/standards/design/layout-grid.md` §Bento dashboard grid).
- Required states, tokens per `docs/standards/design/components.md` §Data-widget states:
  - loading → `shimmer` skeleton over `--color-raised`, reduced-motion → static `--color-raised`
    block (AC-3/AC-8 first paint).
  - unavailable (AC-4/AC-8 error path) → `--color-warn` soft-bg tile + named reason + retry,
    never blank.
  - stale (AC-7) → `--color-text-muted` "stale" badge + `--font-mono` timestamp, content dimmed
    not hidden.
  - pending (AC-5) → "counts pending" chip, never a rendered zero.
  - every tile's footer (AC-3 data-source label) renders `--text-caption` `--color-text-muted`.
- Badges/chips (pending, stale, unavailable) are icon + text, never colour-alone
  (`docs/standards/design/components.md` badge/chip rule, satisfies WCAG SC 1.4.1) — this is the
  token-level backing for the task's own "badges are text + colour, never colour-only" hint.
- No raw principal URN anywhere on this surface — none of this task's own tile bindings carry a
  principal field today, but the rule holds for the whole dashboard page this task renders
  (`docs/design/v1-design-requirements.md` R2; F-D08 finding, `design-assessment-2026-07-09.md`).
  If a future tile binds an owner/actor field, it renders via `EntityRef` (friendly label + mono
  ID chip, R1 bundle), never the raw IRI.
- JTBD success criterion: a user must be able to answer "did anything change?" within 5 seconds
  of landing (`docs/design/jtbd.md` Home/Dashboard) — "Latest published version" and "Draft vs
  published changes" are the load-bearing tiles for this criterion; both must render without a
  blank/unavailable state on a healthy tenant.

**Advisory (uncited, not an acceptance criterion):** `layout-grid.md` names span numbers for
`KpiCard` (3) and the `LineChart`/`DataTable`/`AlertBanner` family (6–12, full) but does not give
a specific span for a dashboard `BarChart` tile at this size — pick a span in the 3–6 range and
keep it consistent across the two bar-chart tiles; not a cited requirement, engineer's call.
