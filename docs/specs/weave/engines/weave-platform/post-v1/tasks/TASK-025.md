...
| AC-7 | WHEN any provider authorize/token endpoint URL is resolved (including tenant-supplied instance hosts, e.g. `https://{instance}.service-now.com`), THE SYSTEM SHALL validate it through the shared `ssrf_guard` (v1-delta §2a, TASK-006 AC-10): HTTPS only, per-type allowed-domain suffix list, resolved IPs outside loopback/link-local (incl. `169.254.169.254`)/RFC-1918/CGNAT/IPv6-ULA; violations → 422 `{"error":"endpoint_not_allowed"}`. Before the denylist check, the resolved address MUST be canonicalised — IPv4-mapped IPv6 literals (e.g. `::ffff:169.254.169.254`) unwrap to IPv4 form, and non-canonical encodings (decimal, octal, hex) are rejected outright rather than parsed. | unit: `test_oauth_endpoints_ssrf_guarded` (parametrised); `test_ssrf_rejects_non_canonical_ip_encodings` (`::ffff:169.254.169.254`, decimal-encoded metadata IP) |
...
| AC-7 | Unit | `test_oauth_endpoints_ssrf_guarded`, `test_ssrf_rejects_non_canonical_ip_encodings` |
