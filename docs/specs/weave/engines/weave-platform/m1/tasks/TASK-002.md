...
| AC-4 | WHEN a component calls the model-routing client with a model tier (`fable`, `sonnet`), THE SYSTEM SHALL route the request to the corresponding Claude model ID (`claude-fable-5`, `claude-sonnet-5`) via the configured provider (Bedrock or Anthropic direct), with no model ID hard-coded outside the routing table. | unit: `test_model_routing_tier_mapping` |
...
| Claude models: fable-5 / sonnet-5 (two tiers) | CLAUDE.md AI/Agents | Routing table maps tier strings — no hard-coded IDs elsewhere in the codebase |
