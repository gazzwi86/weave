...
description: "Create the three Aurora widget-state tables (RLS), the CE-METRICS-1 client with
  pending handling, the SWR read/refresh path, the fixed workspace-default tile set (seeded at
  workspace-create + one-time backfill), and role-appropriate Suggested starters."
...
|----|----------------|--------------|
| AC-1 | WHEN migrations run, THE SYSTEM SHALL create `widget_instances`, `widget_library_items`, `widget_refinements` exactly per m2-delta §4 (columns, CHECKs, indexes), each with `tenant_id` NOT NULL and the M1 RLS policy family applied. | integration: `test_widget_tables_rls_enforced` |
| AC-2 | WHEN a workspace is created, THE SYSTEM SHALL seed the fixed default tile set as `scope='workspace_default'` rows (owner NULL); a one-time backfill migration SHALL seed all existing workspaces. | integration: `test_default_tiles_seeded_on_workspace_create`, `test_backfill_seeds_existing_workspaces` |
...
| AC-9 | WHEN any widget query runs in tenant A's context, THE SYSTEM SHALL return zero rows from tenant B's widget state (extends the M1 cross-tenant-read test to all three tables). | integration: `test_widget_state_cross_tenant_isolation` |
...
# Provisioning
on workspace_create(ws):                     # hook into M1 workspace-create service
  insert DEFAULT_TILES as widget_instances(scope='workspace_default', owner NULL, position=i)
backfill migration: for ws in workspaces without workspace_default rows: same insert
...
**Endpoint:** `GET /api/dashboard/widgets?scope=workspace_default|user`
...
**Endpoint:** `POST /api/dashboard/widgets/{id}/refresh` → 200 `{ "status": "fresh|stale|pending|unavailable", "fetched_at": "…" }`
...
| Provisioning = seed at workspace-create + one-time backfill | HITL decision 2026-07-08 | No lazy first-load creation for workspace_default rows; starters (user-scope) ARE lazy first-load |
| Fixed default is read-only-composed at M2 | E1-S0 AC | No add/remove/reorder UI on workspace_default tiles; members customise via user-scope only |
...
- `test_default_tiles_seeded_on_workspace_create` / `test_backfill_seeds_existing_workspaces` — 6 workspace_default rows, positions 0–5
...
| AC-2 | Integration | `test_default_tiles_seeded_on_workspace_create`, `test_backfill_seeds_existing_workspaces` |
