...
| AC-6 | WHEN widgets render in the grid, workspace-default tiles, suggested starters, and user pins SHALL compose in one grid (defaults first, then pins by position); suggested starters carry the "Suggested" label (TASK-010) and pinning any widget clears Suggested state. | integration: `test_grid_composition_order` |
