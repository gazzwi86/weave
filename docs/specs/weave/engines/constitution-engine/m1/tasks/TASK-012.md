---
type: Task Brief
title: "Task: TASK-012 — Server-Side Layout Persistence"
description: "Persist node drag positions server-side to Aurora per (tenant, workspace, graphId);
  restore on Explorer load; reset on demand. FR-008, net-new Aurora schema from TASK-009."
tags: [graph-explorer, arch, task, m1, aurora, layout]
status: Backlog
priority: Must Have
...
# Task: TASK-012 — Server-Side Layout Persistence

> **Risk flag (FR-008, SS-GE-3):** The `explorer_layout_positions` Aurora schema is net-new (no
> prototype server-side layout existed). TASK-009 designs and approves the schema before this task
...
Covers: FR-008 from [constitution-engine.md §6.1](../../../constitution-engine.md#61-functional-requirements).
...
|---------|------|------------------|---------|
| Sequence | `../../tech-spec/business-process-explorer.md` | `#layout-persist-flow` | Drag-end UPSERT to Aurora with SET LOCAL RLS + optimistic retry |
| State | `../../tech-spec/business-process-explorer.md` | `#layout-load-state` | Clean → Dirty → Persisting → RetryPending layout lifecycle |
| Data Model | `../../tech-spec/data-model-explorer.md` | `#explorer-layout-positions` | Approved `explorer_layout_positions` DDL + RLS this task implements |

### Design Decisions

| Decision | Reference | Impact on This Task |
|----------|-----------|---------------------|
| Server-side layout persistence per (tenant, workspace, graphId) — not localStorage (D2, FR-008) | [constitution-engine.md §6.5](../../../constitution-engine.md#65-key-design-decisions) | Every read/write must be scoped by all three keys; no per-browser storage path |
| Aurora schema: `explorer_layout_positions` with RLS policy `tenant_id = current_setting('app.current_tenant_id')::uuid` | TASK-009 deliverable (AC-4) | `SET LOCAL app.current_tenant_id` must be called inside every `async with db.begin()` block before any query; failing to do so bypasses RLS |
...
| Failed layout save → optimistic hold + retry + non-blocking toast; never silently dropped (FR-008) | [constitution-engine.md §6.1](../../../constitution-engine.md#61-functional-requirements) | Client retry logic: 3 attempts with [2 s, 4 s, 8 s] delays; toast on exhaustion; all tunable |
| `locked` field reserved for M2 Saved Views; default `false` at insert; not writable via this task's API | [constitution-engine.md §5.1](../../../constitution-engine.md#scope) | This task's POST endpoint does not accept a `locked` field; DB default handles it |
...
- **blocked_by:** [TASK-009 (Aurora schema approval required before writing any DB code),
  TASK-010 (canvas drag events must exist for client-side integration)]
- **unlocks:** []
...
- **Estimated cost:** ~$1.00 (claude-fable-5 at time of writing)
