---
type: Task Brief
title: "Task: TASK-009 — SPIKE: Cytoscape 10k-node Benchmark + Aurora Layout Schema Design"
description: "Benchmark Cytoscape.js + fcose at 10k nodes to gate OQ-01; produce WebGL escape-hatch
  go/no-go (OQ-05); design and approve the Explorer Aurora layout schema that unblocks TASK-012."
...
# Task: TASK-009 — SPIKE: Cytoscape 10k-node Benchmark + Aurora Layout Schema Design

**Owner:** Architect · **Type:** Benchmark / Decision Spike

## Story

**Epic:** [EPIC-013](../../../constitution-engine.md#epic-013--whole-company-canvas-force-mode--m1)
**Status:** Backlog
**Priority:** Must Have
...
**So that** the M1 gate has a signed go/no-go on the WebGL escape hatch (OQ-05) and TASK-012 has
...
| AC-2 | WHEN the benchmark report shows initial render ≤ 8 s at 10k and drag ≥ 60 fps at ≤ 1,000 visible nodes (p95, tunable), THE SPIKE SHALL be marked "go" and TASK-010 AC-7 (performance) SHALL stand as written. | `benchmark_go_decision_signed` (Architect sign-off) |
| AC-3 | WHEN the benchmark report shows initial render > 8 s at 10k OR drag < 60 fps at ≤ 1,000 visible nodes, THE Architect SHALL produce a signed OQ-05 decision naming the WebGL renderer (sigma.js or G6) and TASK-010 AC-7 SHALL be suspended until the renderer decision is approved. | `benchmark_no_go_oq05_decision_signed` (Architect sign-off) |
...
| AC-5 | WHEN the schema design is approved by the Architect, THE approved SQL migration SHALL be committed to the repository as a migration file and TASK-012 SHALL be marked unblocked. | `layout_schema_migration_committed` (git log confirms file) |
...
STEP 4: Decision tree
  if p95_load_10k <= 8_000 AND p95_drag_fps >= 60:
    decision = "go"
    file: ADR-014.md { renderer: "cytoscape+fcose", status: "confirmed" }
  else:
    decision = "no-go"
    file: ADR-014.md {
    ...
    suspend: TASK-010 AC-7         # performance AC held until renderer decided
...
STEP 5: Aurora layout schema design
  ...
  # Note for TASK-012: FastAPI middleware must call
...
The migration SQL produced here becomes the schema for TASK-012's Aurora endpoints.

### Diagram References

| Diagram | File | Relevant Section | Summary |
|---------|------|------------------|---------|
| Sequence | `../../tech-spec/business-process-explorer.md` | `#graph-load` | CE-READ-1 paginated graph-load sequence the benchmark fixtures emulate |
| State | `../../tech-spec/business-process-explorer.md` | `#benchmark-decision` | Go/no-go state machine driving the OQ-01 SPIKE and the ADR-014 flip |
| Data Model | `../../tech-spec/data-model-explorer.md` | `#layout-schema` | Normative `explorer_layout_positions` DDL + RLS this task designs and approves |
| Performance | `../../tech-spec/architecture-explorer.md` | `#10k-node-performance-budget` | 10k-node render/drag budget the benchmark measures against |

### Design Decisions

| Decision | Reference | Impact on This Task |
|----------|-----------|---------------------|
| Cytoscape.js + fcose for force layout (prototype-proven); WebGL escape hatch (sigma.js/G6) if 10k targets fail (SS-GE-1) | [constitution-engine.md §6.5](../../../constitution-engine.md#65-key-design-decisions) | Benchmark must use the exact fcose params from `prototype-findings.md`; deviation from prototype params invalidates the comparison |
| Force canvas at 10k nodes is an unverified target (OQ-01 / OQ-05) | [constitution-engine.md §6.2](../../../constitution-engine.md#62-non-functional-requirements) | Performance ACs in TASK-010 are gated on this SPIKE; never assert 10k targets as settled before AC-2 or AC-3 is signed |
| Explorer-owned Aurora tables, tenant + workspace scoped, server-side layout (D2) | [constitution-engine.md §6.5](../../../constitution-engine.md#65-key-design-decisions) | Schema must carry `tenant_id` + `workspace_id` on every row; row-level security required |
...
- OQ-01 / OQ-05 decision filed as `decisions/ADR-014.md` (Architect creates)
...
- `git log` confirms migration file committed before TASK-012 is started (AC-5)
...
- **blocked_by:** []
- **unlocks:** [TASK-010 (AC-7 performance gate), TASK-012 (Aurora schema sign-off required to start)]
...
- The `SET LOCAL app.current_tenant_id` pattern must be documented in the schema design notes
  for TASK-012: `SET LOCAL` is connection-scoped; SQLAlchemy async connection pools reuse
  connections, so `SET LOCAL` must be called inside every `async with session.begin()` block.
- File the OQ-01 / OQ-05 decision as `decisions/ADR-014.md` so TASK-010 and TASK-012
