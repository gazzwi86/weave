---
type: Task Brief
title: "Task: TASK-010 — Whole-Company Force Canvas + Navigation"
...
created: 2026-07-01
blocked_by: [TASK-009]
unlocks: [TASK-011, TASK-012, TASK-013]
...
# Task: TASK-010 — Whole-Company Force Canvas + Navigation
...
|----|------------------|--------------|
| AC-1 | WHEN an authenticated viewer opens the Explorer, THE SYSTEM SHALL call CE-READ-1 `GET /api/sparql?version=latest&page=N` (paginated until `has_more_pages` is false) and render all returned nodes and edges in a Cytoscape.js + fcose force canvas; fcose SHALL only randomise positions for nodes that have no saved layout position from TASK-012. | `test_canvas_loads_draft_graph_via_ce_read_1` |
...
| AC-8 | WHEN the M1 force canvas renders its **bounded visible-node set** (≈1–2k nodes; the whole-company 10k-at-once target was retired for M1 per ADR-014, human sign-off 2026-07-05), THE SYSTEM SHALL complete first interactive render within ≤ 3 s at ≤ 1k visible nodes (p95, tunable) on reference hardware (desktop Chrome latest), and beyond the visible bound SHALL rely on server-side pagination / level-of-detail / domain-focus drill-in rather than rendering all nodes at once. The retired 10k-single-canvas threshold moves to the **pre-v1 WebGL migration** (ADR-014 preferred renderer), not M1. | `test_canvas_load_performance_p95_1k`, `test_canvas_bounded_visible_node_set` |
...
|---------|------|------------------|---------|
| Sequence | `../../tech-spec/business-process-explorer.md` | `#canvas-initial-load` | Full canvas boot: palette + saved positions + paginated CE-READ-1 load |
| State | `../../tech-spec/business-process-explorer.md` | `#canvas-load-state` | Load state machine (loading → rendered / CE-error empty-state) |
| Data Model | `../../tech-spec/data-model-explorer.md` | `#bpmo-node-kinds` | 13 BPMO kinds + grey fallback the palette colours nodes by |
| Component | `../../tech-spec/architecture-explorer.md` | `#level-3-component--spa-canvas-module` | SPA Canvas Module boundary and renderer-adapter seam |

### Design Decisions

| Decision | Reference | Impact on This Task |
|----------|-----------|---------------------|
| Cytoscape.js + fcose for the M1 bounded force canvas (TASK-009 signed off 2026-07-05: 10k-at-once retired for M1, WebGL preferred pre-v1) | [ADR-014](../../decisions/ADR-014-render-engine.md) | Use fcose with prototype params verbatim; build against the render-adapter boundary so the pre-v1 WebGL swap is render-layer-only; AC-8 targets the bounded ≤1k-visible render, not 10k |
| Node shape = single ellipse in M1; kind→shape mapping deferred (OQ-08) | [constitution-engine.md §6.5](../../../constitution-engine.md#65-key-design-decisions) | Do not implement per-kind shapes; all nodes are ellipses |
| CE error → empty-state; no partial render (FR-001) | [constitution-engine.md §6.1](../../../constitution-engine.md#61-functional-requirements) | Cytoscape must NOT be initialised if elements is empty due to error; prevents phantom canvas state |
| Semantic zoom thresholds tunable (not hard-coded literals) (FR-005) | [constitution-engine.md §6.1](../../../constitution-engine.md#61-functional-requirements) | Threshold values must come from a config object; no magic numbers in component |
| No global keyboard capture for Cmd+K / Cmd+0 (FR-007) | [constitution-engine.md §6.1](../../../constitution-engine.md#61-functional-requirements) | Key listener must check `cy.container().contains(document.activeElement)` before firing |
...
- **blocked_by:** [TASK-009 (AC-8 performance AC gated on SPIKE sign-off)]
- **unlocks:** [TASK-011, TASK-012, TASK-013]
