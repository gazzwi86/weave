---
type: Task Brief
title: "Task: TASK-024 — Side-Panel Property Edit + Delete Node/Edge + Concurrency Guard"
...
blocked_by: [TASK-023]
unlocks: [TASK-030]
adr_refs: [ADR-014-render-engine, ADR-019-edit-attribution-principal-iri, ADR-021-m2-concurrency-client-drift-guard]
...
# Task: TASK-024 — Property Edit + Delete + Concurrency Guard
...
Covers: FR-021, FR-022 ([constitution-engine.md §6.1](../../../constitution-engine.md#61-functional-requirements)).
Reuses TASK-023's Edit Controller and write proxy — this task adds `update_node`,
...
> constitution-engine.md, ADR-021) is therefore implemented as a **GE-side optimistic drift guard**
...
|----|------------------|--------------|
| AC-1 | WHERE a BA/ontologist has a draft node spotlighted, WHEN they save edited label/comment/typed props THE SYSTEM SHALL commit via the write proxy (`update_node`); CE writes PROV-O + PLAT-AUDIT-1 (actor per ADR-019 — proxy-injected, nothing client-side). | `test_update_node_commits_via_write_proxy` |
...
```
# Reuses TASK-023: commitOp(op, …) with rollback; write proxy unchanged.
...
function openEdit(node, ctx):
  ctx.editBase = ctx.draftHead            # version_iri GE already tracks (last 201 response
                                          # or graph-load state; TASK-026's poll also updates it)
...
No new endpoints — `POST /api/proxy/operations/apply` (TASK-023) with ops `update_node`,
...
draft head GE already tracks (graph-load / last 201 / TASK-026 poll); at worst one
...
|---------|------|---------|---------|
| Component delta | `../../tech-spec/m2-delta-explorer.md` | §6 | Edit Controller → write proxy path this task extends |
| State | `../../tech-spec/business-process-explorer.md` | side-panel states | M1 panel state machine gaining edit/conflict states |

### Design Decisions

| Decision | Reference | Impact |
|----------|-----------|--------|
| Concurrency = GE-side optimistic drift guard for M2; server-side guard is CE-WRITE-1's planned additive v1 `expected_version` → `409 {current_version_iri}` | ADR-021; contracts.md CE-WRITE-1 (pinned 2026-07-08) | FR-021's "409" is UX language in M2; at v1, swap the drift re-check for the real conditional write — no bespoke guard meanwhile |
...
| Actor attribution proxy-side | ADR-019 | Client code never touches actor |
| Handle-hiding is UX, not security | constitution-engine.md §6.2 | AC-8 dual assertion, same as TASK-023 AC-7 |
...
| AC-8 | Integration | reuse TASK-023 pattern against panel affordances |

## Dependencies

- **blocked_by:** [TASK-023 (Edit Controller + write proxy)] — unchanged by the reframe
- **unlocks:** TASK-030; optionally enriches TASK-027 (E10-S2 inline edit shortcut)
- **Soft:** TASK-026's poll keeps `draftHead` fresher (narrower drift window) — beneficial,
...
- `currentDraftHead`: prefer the head GE already holds (updated by every 201 and by TASK-026's
