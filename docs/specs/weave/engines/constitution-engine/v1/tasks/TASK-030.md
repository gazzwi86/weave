---
type: Task Brief
title: "Task: TASK-030 — M2 Release-Gate Suite (isolation, a11y, perf, invariants)"
description: "Terminal M2 task: two-tenant cross-surface isolation suite, axe/Lighthouse gates
  on all M2 panels, perf traces for the 300ms/10k budgets, invariants-explorer.md verify-by execution,
...
milestone: v1
created: 2026-07-08
blocked_by: [TASK-022, TASK-024, TASK-026, TASK-027, TASK-028, TASK-029]
unlocks: []
adr_refs: [ADR-018-impact-traversal-predicate-closure, ADR-019-edit-attribution-principal-iri]
...
# Task: TASK-030 — M2 Release-Gate Suite

## Story

**Epic:** cross-epic (M2 exit gate — [constitution-engine.md §Roadmap M2](../../../constitution-engine.md#m2--fast-follow-editing--overlays--async))
...
GE-CANVAS-1 conformance report (produced by TASK-029, referenced here), coverage/mutation
...
| AC-4 | WHEN the perf-trace job runs at the 10k fixture, THE SYSTEM SHALL evidence: filter/overlay/badge apply ≤ 300 ms p95 (TASK-020/021/027), view save ≤ 800 ms p95 (TASK-025 AC-9 measurement), proxy overheads within m2-delta §4 targets. | CI `perf-m2` job |
| AC-5 | WHEN the invariants job runs, THE SYSTEM SHALL execute every M2 verify-by selector in `tech-spec/invariants-explorer.md` (greps + named-test presence), pinning any still-indicative file paths to real ones, and fail on any unmet invariant. | CI `invariants-check` job |
| AC-6 | WHEN the gate bundle builds, THE SYSTEM SHALL emit one artefact directory: isolation report, axe/Lighthouse outputs, perf traces, invariants result, GE-CANVAS-1 conformance report (from TASK-029), coverage + mutation numbers — each file named and dated. | `test_gate_bundle_completeness` |
...
# Invariants runner (AC-5) — small script, reads invariants-explorer.md, executes each verify-by
for line in parse(invariants-explorer.md, section="M2 delta") + section("M1"):
  run(line.verify_by)   # grep pattern | test-name presence in suite manifests
  record(pass/fail)
# indicative paths: resolve against the real tree, update invariants-explorer.md selectors in this PR
...
|---------|------|---------|---------|
| Quality table | `../../tech-spec/architecture-explorer.md` | Quality Attributes | M1 gate rows this suite extends to M2 |
| Delta targets | `../../tech-spec/m2-delta-explorer.md` | §4–§7 | The budgets and testing delta this suite evidences |
| Invariants | `../../tech-spec/invariants-explorer.md` | all | The checklist AC-5 executes |

### Design Decisions

| Decision | Reference | Impact |
|----------|-----------|--------|
| Isolation is a release gate, tested cross-surface in one suite | constitution-engine.md §6.2 required test | One fixture, eight surfaces — not eight bespoke fixtures |
| Invariants get an executable runner, not manual review | Arch Law 10; invariants-explorer.md header | The runner also PINS indicative paths — invariants-explorer.md leaves this task with real selectors |
...
- Invariants: every M2 line in invariants-explorer.md executed; every M1 line re-executed
...
- **blocked_by:** [TASK-022, TASK-024, TASK-026, TASK-027, TASK-028, TASK-029] (terminal —
  everything M2 ships is in scope; TASK-020/021/023/025 are transitively covered via these)
...
- [ ] invariants-explorer.md selectors pinned to real paths (committed in this PR)
...
- axe on GraphCanvas states: mount the component in the three states (empty/error/loading) in
  a bare route — the standalone-mount work from TASK-029 gives you this for free.
