---
type: Task Brief
title: "Task: TASK-028 — Closure Config Wiring + Drift Guard + Pinned Impact Overlay"
description: "Ships the ADR-018 oq09_predicate_closure config with boot-time drift guard
...
# Task: TASK-028 — Closure Config + Drift Guard + Pinned Impact Overlay

## Story

**Epic:** [EPIC-016](../../../constitution-engine.md#epic-016--visual-overlays--m2) (E4-S3) +
ADR-018 productionisation
...
[ADR-018](../../decisions/ADR-018-impact-traversal-predicate-closure.md).
**Scope correction (2026-07-08 red-team):** M1 TASK-013 closed **without** its OQ-09-gated
AC-6/AC-7 — no traversal client exists in the shipped M1 code. This task therefore OWNS the
traversal client too: the property-path SELECT walk (M1 TASK-013 AC-6/AC-7, incl. their named
...
|----|------------------|--------------|
| AC-1 | WHEN the app builds, THE SYSTEM SHALL include the 13-entry `oq09_predicate_closure` default exactly per ADR-018 (9 forward, 4 inverse) in checked-in config; no predicate IRI SHALL appear as a literal in traversal code. | `test_closure_config_matches_adr_005`, grep gate per invariants-explorer.md |
...
| AC-4 | WHEN the pinned trace's source node is deleted (TASK-024 delete or poll refresh reports it gone), THE SYSTEM SHALL auto-clear the pinned overlay with a notice. | `test_pin_autoclears_on_source_delete` |
...
```
# Config (AC-1) — checked-in default, shape per ADR-018
config.oq09_predicate_closure = [ { predicate, orientation } × 13 ]   # copy ADR-018 table
...
# Pinned impact overlay (AC-3..7) — TASK-021 engine, highlight channel
...
# Walk composition (shared with M1 TASK-013 traversal client — one function, two directions)
...
|---------|------|---------|---------|
| Component delta | `../../tech-spec/m2-delta-explorer.md` | §6 | Overlay engine channel model; traversal client path |
| C4 L3 | `../../tech-spec/architecture-explorer.md` | Component — SPA Canvas Module | Impact-Traversal Client + Config Store this task wires |

### Design Decisions

| Decision | Reference | Impact |
|----------|-----------|--------|
| 13-entry directed closure; one list, two walk directions | ADR-018 | AC-1/AC-6 are direct enforcement; the mirror property-test is the cheap proof the two paths share one config |
| Drift guard fails loud, missing-only (extra CE predicates OK) | ADR-018 | Additive ontology growth must not brick traversal; only a closure entry CE no longer serves is drift |
| Static app config; per-tenant override deferred | ADR-018 (20Q Q8) | No PLAT-SETTINGS-1 read here — adding one is scope creep |
| Pin = highlight channel (not colour, not badge) | TASK-021 engine channels | Coexists with completeness badges + domain colour; diff blocks it via colour-group activation clearing highlights — test AC-7 |

## Test Requirements

### Unit (minimum 4)

- `should ship closure config identical to ADR-018 table (snapshot test)`
...
| AC | Type | Test |
|----|------|------|
| AC-1 | Unit + grep gate | snapshot test + invariants-explorer.md grep |
...
- **blocked_by:** [TASK-021 (overlay engine + channels)]
- **unlocks:** TASK-030; delivers the orphaned M1 TASK-013 AC-6/AC-7 (traversal client —
  see scope correction above; M1 TASK-013 is `done` in progress.json without them)
...
- [x] Design decisions noted (ADR-018 enforcement points explicit)
...
- [ ] invariants-explorer.md grep gates pass (no predicate literals; drift-guard test present)
...
- The config snapshot test should literally embed the ADR-018 JSON — if someone edits the
...
- Highlight styling from `docs/standards/design/data-viz.md` trace tokens; the pin legend is
  the accessible surface (badge/aria pattern same as TASK-027).
