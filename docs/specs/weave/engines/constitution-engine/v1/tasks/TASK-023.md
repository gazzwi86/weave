---
type: Task Brief
title: "Task: TASK-023 — Edit Controller + Write Proxy + Quick-Add Node + Draw Edge"
description: "CE-WRITE-1 write proxy with server-side principal_iri attribution (ADR-019);
...
# Task: TASK-023 — Edit Controller + Write Proxy + Quick-Add + Draw Edge

## Story

**Epic:** [EPIC-017](../../../constitution-engine.md#epic-017--visual-editing-on-canvas--m2)
...
Covers: FR-019, FR-020 ([constitution-engine.md §6.1](../../../constitution-engine.md#61-functional-requirements)).
Delivers the **write proxy** (`POST /api/proxy/operations/apply`) and the **Edit Controller**
(optimistic apply + rollback) that TASK-024 (property edit / delete) and TASK-029 (GE-CANVAS-1
write-back) reuse.
...
| AC-9 | WHEN a node is selected on the draft or published canvas, THE SYSTEM SHALL open the glass inspector panel (Properties / Edges / PROV tabs + Edit entry, per Design requirements D-1..D-7) replacing the M1 flat side panel; the inspector's Edit entry SHALL be the sole affordance opening this task's Edit Controller. | `test_node_select_opens_glass_inspector`, `test_inspector_edit_entry_opens_edit_controller` |
...
POST /api/proxy/operations/apply:
  ...
  body = { operations: req.body.operations,
           actor: principal,                        # verbatim — never built from sub (ADR-019)
...
p95: ≤ 100 ms proxy overhead on CE's ≤ 800 ms write budget (m2-delta-explorer.md §4).

### Diagram References

| Diagram | File | Section | Summary |
|---------|------|---------|---------|
| Component delta | `../../tech-spec/m2-delta-explorer.md` | §6 | Edit Controller → CE-Write Proxy → CE-WRITE-1 path |
| Sequence | `../../tech-spec/business-process-explorer.md` | edit-commit flow (M1 doc: read flows; write flow mirrors spotlight fetch pattern with rollback) | Optimistic apply / rollback lifecycle |

### Design Decisions

| Decision | Reference | Impact |
|----------|-----------|--------|
| `actor` = JWT `principal_iri` claim, server-side, verbatim; spoof guard | [ADR-019](../../decisions/ADR-019-edit-attribution-principal-iri.md) | AC-1/AC-2 are the enforcement; invariants-explorer.md greps this handler |
...
| Canvas handle-hiding is UX, never security | constitution-engine.md §6.2 Security | AC-7's second test asserts server-side rejection independent of UI state |
| Edgehandles prototype params | constitution-engine.md §Product Context | Port params from prototype into `config.edgehandles_params`; tunable |
| Draft-only writes in M2 | FR-019–022; ge-canvas-1.md rule 4 | Proxy pins `target: "draft"`; version-pinned canvas mode disables the controller (flag built in TASK-022 — coordinate if TASK-022 lands later: build the flag here if first) |
...
| AC-9 | Integration + E2E | inspector-open + Edit-entry tests |

## Dependencies

- **blocked_by:** none within M2 (consumes M1 canvas/adapter; builds its own proxy route).
  Coordinate with TASK-022 on the shared canvas-mode (read-only) flag — whichever lands first
  builds it.
- **unlocks:** TASK-024 (update/delete reuse controller + proxy), TASK-029 (GE-CANVAS-1
...
- [x] Design decisions noted (incl. ADR-019 enforcement points)
...
- [ ] Edit Controller API documented for TASK-024/029 (docstring level)
- [ ] Conventional commit(s); PR references this task and EPIC-017
...
|---|---|---|
| D-1 | WHEN a node is selected on the canvas, THE SYSTEM SHALL open a glass inspector panel — not the flat side panel M1 built — showing four sections: Properties, Edges, PROV, and an Edit entry point. | R4 (`docs/design/v1-design-requirements.md`, "Node select → glass inspector (properties, edges, PROV, edit, expand-neighbours per research pattern 8)"); F-D17 (Major — "No inspector side-panel on node select... a phantom empty panel sits clipped at the bottom-right corner"); `docs/design/jtbd.md` Constitution → Explore success criteria ("click → inspector (props/edges/PROV, edit entry)") |
| D-2 | The inspector's expand-neighbours action SHALL invoke the existing M1 neighbour-expansion function (`onExpandNeighbours`, constitution-engine/m1/tasks/TASK-013.md AC-3/AC-4) — this task SHALL NOT build a second expansion implementation. | `docs/design/research/graph-canvas-ux-patterns.md` pattern 8 ("Inspector doubles expansion launch point — Bloom double-click → Inspector → expand") |
| D-3 | The inspector's Edit entry point SHALL be the affordance that opens this task's own optimistic Edit Controller — one edit surface, not a second. | R4 "edit" bullet; this brief's own AC-3/AC-4 (quick-add + rollback lifecycle) |
| D-4 | Edit affordances (inspector Edit entry, quick-add, draw-edge) SHALL render only for BA/ontologist roles on the draft canvas, never for viewer role or a pinned published version — restated here for design traceability. | This brief's own AC-7 |
| D-5 | Properties and edges in the inspector SHALL show friendly label + mono-ID chip; the raw IRI appears only under an "Advanced / technical details" disclosure, visible only under the `ontologist` role claim — the glass inspector inherits the M1 side panel's rule unchanged. | F-D08 (Minor — raw machine identity leaking); `docs/standards/design/components.md` §Side panel / inspector ("Honours the model-hiding contract: raw IRIs only under an 'Advanced / technical details' disclosure") |
| D-6 | The inspector SHALL be fully keyboard-navigable and ARIA-labelled. | `docs/standards/design/components.md` §Side panel / inspector ("Fully keyboard-navigable and ARIA-labelled (not subject to the canvas v1 exemption)") |
| D-7 | Stacking layer: `--z-panel`. Surface: glass (translucent fill + blur + `--shadow-overlay`) for this canvas-context inspector specifically. | `docs/standards/design/tokens.md` zIndex table (`--z-panel` — "side panel / inspector"); `docs/design/visual-direction.md` ("glass stays canvas/overlay-only per the design system") |

### Gaps

- **Flat-vs-glass discrepancy:** `docs/standards/design/components.md` §Side panel / inspector
  specifies a flat surface (`--color-surface`, `--shadow-panel`) as the default for that
  component, while `docs/design/visual-direction.md`'s V4 recipe specifies glass for the canvas
  node inspector specifically. Both sources are cited correctly above — the reconciliation is
  that the Explorer canvas inspector is the specific glass exception the direction doc carves
  out; the Instance-browser inspector (R3, a different surface) stays flat per components.md's
  default. Flagging so the InspectorPanel organism (once built under R13/Storybook) ships as a
  canvas-context glass variant, not a global re-skin of the one component.
- **AC coverage gap — RESOLVED (architect, 2026-07-09):** D-1 through D-7 describe UI scope
  (the glass inspector shell, its Edges/PROV data, the expand-neighbours entry point) that had
  no AC row when first flagged. Resolution: the inspector shell belongs to THIS task (it is the
  Edit Controller's entry surface — splitting them would create a circular dependency), covered
  by the added AC-9 with its test-mapping row. Original flag kept for provenance.
