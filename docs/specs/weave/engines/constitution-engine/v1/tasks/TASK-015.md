---
type: Task
title: "Task: TASK-015 — Structured Model Import: ArchiMate Exchange Format + BPMN"
...
Two deterministic (no-LLM) extractor plugins on the TASK-012 spine: ArchiMate Exchange XML and
...
down. OUT: diagram images (TASK-016), cross-notation merge (TASK-018), any visual mapping UI.
...
No new endpoints — rides TASK-012's spine (upload with `kind=archimate|bpmn` detection,
...
- **blocked_by**: TASK-012 (spine)
- **unlocks**: TASK-014 (its corpus chunker reuses this task's notation XML parse — ADR-011
  "no second parser"), TASK-018 (cross-notation reconciliation needs ≥ 2 notations landing),
  TASK-019
...
- The intermediate RDF keeps original notation types as annotations (`weave:sourceType
  ...
  TASK-018's reconciler can trace cross-notation origins.
