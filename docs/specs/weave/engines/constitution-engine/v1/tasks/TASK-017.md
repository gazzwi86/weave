---
type: Task
title: "Task: TASK-017 — Structured-Data Import: R2RML + RML (morph-kgc), OCEL Reference Mapping"
...
No new endpoints — TASK-012 spine (upload accepts `mapping_artefact_iri` alongside the dataset;
...
| OCEL ships as a reference mapping file, not code | Data not code — amendable like the TASK-015 tables; per-qualifier gaps land as flagged rows | ADR-010 |
...
- **blocked_by**: TASK-012 (spine; corpus artefact storage for mappings)
- **unlocks**: TASK-019
...
- The "bulk accept" UX for thousands of row-proposals: accept-all-unflagged is a client loop
  over TASK-012 accepts in v1 (per-proposal HITL stays intact — flagged rows always need
