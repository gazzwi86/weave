...
| AC-007-03 | WHEN `owl_inconsistencies` is served THE SYSTEM SHALL report the publish-time reasoner result stored with the latest version where one exists; WHILE no reasoner ships (post-v1, EPIC-008) THE SYSTEM SHALL serve an explicit `{"pending": true}`-style not-computed marker — never zeros, never live reasoning (honesty rule, m2-delta §7). |
| AC-007-04 | WHEN `draft_published_delta` is computed THE SYSTEM SHALL reuse the M1 **internal graph-diff function** applied to the draft graph vs the latest published graph — NOT the CE-DIFF-1 HTTP endpoint (which accepts only two PUBLISHED version IRIs) and not a re-implementation. |
...
GET /api/metrics/ontology:
    ...
    delta   = graph_diff(draft_graph, latest_published_graph).counts
              # M1 internal diff function — NOT the CE-DIFF-1 endpoint
              # (endpoint takes two PUBLISHED version IRIs; draft is not one)
    ...
    owl     = latest.publish_reasoner_result                 # stored at publish, never live
              (or {"pending": true} — no reasoner exists until post-v1 EPIC-008; never zeros)
...
| Reuse the M1 internal diff function (draft vs latest published) for the delta | The diff logic is already correct and edge-inclusive; a second implementation would drift. The CE-DIFF-1 **endpoint** cannot be reused — it accepts only published version IRIs, and the delta's left side is the draft | CE ADR-002 (M1 diff design), contracts CE-DIFF-1 |
| SHACL severity counts and `owl_inconsistencies` may be "pending" | Mirrors TASK-006: never serve stale/absent counts as fresh zeros; `owl_inconsistencies` has NO producer until the post-v1 reasoner (EPIC-008) | TASK-006 AC-006-04, m2-delta §7 |
...
| Unit | should serve zeros for an empty tenant graph (counts/delta only — pending markers still apply to report-backed fields) | AC-007-06 |
| Unit | should serve pending marker for owl_inconsistencies when no stored reasoner result exists | AC-007-03 |
