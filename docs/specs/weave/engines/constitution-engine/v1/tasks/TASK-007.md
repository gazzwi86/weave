---
type: Task
title: "Task: TASK-007 — SKOS Cross-Notation Reconciliation"
...
into the TASK-001 proposal store for human confirmation, merge execution as CE-WRITE-1 ops
...
infra (uses TASK-003's if present, string similarity as the floor).
...
def reconcile(tenant, scope=recent_ingests):
    ...
    for (a, b) in same_kind_pairs(entities):             # blocked: kind mismatch
        s = similarity(a, b)                             # normalized label edit-distance
                                                         ...
                                                         # embeddings exist (TASK-003), else
...
# acceptance runs through TASK-001's accept -> prospective SHACL -> CE-WRITE-1;
# conflicting single-valued props fail validation there -> 422 -> AC-007-04 surfaces it.
```

## API Contracts

No new endpoints — merge proposals are TASK-001 proposals (kind=`merge`), reviewed on the
import page (TASK-008) / chat. Mutation: **CE-WRITE-1** only; CE-READ-1 verifies the collapsed
view.
...
|---|---|---|
| Merge = ordinary proposal in the TASK-001 store | Human confirmation, 422 surfacing, audit — all inherited; no bespoke merge UI flow | v1-delta §1 |
...
| Similarity floor = string metrics; embeddings only if TASK-003 present | Keeps 007 buildable without corpus infra; embeddings are an accuracy upgrade, not a dependency | ponytail ladder |
...
- **blocked_by**: TASK-001 (proposal store), TASK-004 (needs ≥ 2 notations actually landing to
...
- [x] Merge-as-proposal flow pinned (TASK-001)
- [ ] TASK-001 + TASK-004 merged (DAG)
