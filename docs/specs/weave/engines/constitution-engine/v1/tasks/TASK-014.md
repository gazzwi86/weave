---
type: Task
title: "Task: TASK-014 — Document Corpus Store: Chunking, Embeddings, Retrieval, NL Citations"
...
milestone: v1
created: 2026-07-08
blocked_by: [TASK-012, TASK-013, TASK-015]
unlocks: [TASK-019]
adr_refs: [ADR-011]
...
prompt-side lands in TASK-013) · Contracts: [contracts.md](../../../../contracts.md) (CE-READ-1
...
— **reusing TASK-015's notation parse for ArchiMate/BPMN XML**, the "no second parser" rule,
...
```text
on_ingest_commit(job):                       # hook after TASK-012 marks proposals accepted
    passages = split(job.artefact)           # dispatch: xml|prose|fallback (pin 1a)
                                             # xml = TASK-015's parsed notation model
                                             # prose = TASK-013's heading tree — no new parser
...
Minimum: 5 unit, 5 integration, 1 E2E-adjacent (rides TASK-019's E2E).

| Layer | Scenario (`should X when Y`) | AC |
|---|---|---|
| Unit | should split prose by headings to ~512 tokens with overlap + locators | AC-003-01 |
| Unit | should split XML per element with parent context prepended (consuming a TASK-015 parsed-model fixture — no XML parsing in this module) | AC-003-01 |
...
- **blocked_by**: TASK-012 (artefact store, prov links), TASK-013 (prose/document structure
  parse the chunker reuses; source_span locators in proposals), TASK-015 (**notation XML parse**
  — the ArchiMate/BPMN per-element splitter consumes TASK-015's parsed intermediate model, NOT
  a new XML parser; ADR-011 "no second parser" reconciled: prose parse lives in TASK-013, XML
  parse lives in TASK-015, this task owns only chunk assembly + fallback)
- **unlocks**: TASK-019 (epic E2E asserts a citation end-to-end)
...
- [x] Locators flow from TASK-013 (source_span requirement written into its brief)
- [ ] TASK-012 + TASK-013 + TASK-015 merged (DAG — TASK-015 supplies the XML parse the
