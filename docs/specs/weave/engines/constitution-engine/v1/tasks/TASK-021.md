---
type: Task Brief
title: "Task: TASK-021 — Overlay Engine + Heatmap + Domain Colouring"
...
created: 2026-07-08
blocked_by: [TASK-020]
unlocks: [TASK-022, TASK-027, TASK-028]
...
# Task: TASK-021 — Overlay Engine + Heatmap + Domain Colouring

## Story

**Epic:** [EPIC-016](../../../constitution-engine.md#epic-016--visual-overlays--m2)
...
Covers: FR-015, FR-018 ([constitution-engine.md §6.1](../../../constitution-engine.md#61-functional-requirements)).
Delivers the **Overlay Engine** (registration, mutual exclusion, legend) that TASK-022 (diff),
TASK-027 (completeness), and TASK-028 (pinned impact) plug into.
...
```
# Overlay Engine — single owner of node/edge colour + badge state (via adapter, ADR-014)
...
function heatmapApply(adapter, { dimension, config }):
  ...
  adapter.batch(() =>
    for n in adapter.allNodes():
      ...
      colour = mapping.values[normalise(v)] ?? (unmatched++, config.grey)  # --color-heat-none token (D-2)
...
|---------|------|---------|---------|
| Component delta | `../../tech-spec/m2-delta-explorer.md` | §6 | Overlay Engine position; consumers (diff, completeness, pinned impact) |
| C4 L3 | `../../tech-spec/architecture-explorer.md` | Component — SPA Canvas Module | Adapter boundary all colour ops go through |

### Design Decisions

| Decision | Reference | Impact |
|----------|-----------|--------|
| Heatmap uses prototype value→colour mappings, free-text fields | FR-015, constitution-engine.md §6.5 | Mappings are config, ported from the prototype — do not invent new value vocabularies |
...
| Overlay Engine is the ONLY writer of colour state | m2-delta-explorer.md §6 | TASK-022/027/028 register overlays; they never call `adapter.setColour` outside their `apply()` |
| Adapter-only canvas access | ADR-014-render-engine | Same as TASK-020 |
...
- **blocked_by:** [TASK-020 (panel shell + legend mount point; filter-state interplay)]
- **unlocks:** TASK-022 (diff overlay registers here), TASK-027 (completeness overlay),
  TASK-028 (pinned impact overlay)
...
- [ ] Overlay registration API documented for TASK-022/027/028 (docstring level)
- [ ] Conventional commit(s); PR references this task and EPIC-016
...
|---|---|---|
| D-1 | The active overlay's legend SHALL render inside the shared canvas legend shell built in TASK-020 (bottom-left, collapsible) — this task owns no second floating legend surface. | R4 (`docs/design/v1-design-requirements.md`); this task's own `blocked_by: [TASK-020]` dependency |
| D-2 | The heatmap "unmatched" grey and any domain-colouring fallback SHALL resolve through the design system's named unmatched/fallback colour token, not a literal hex value. This task's own pseudocode (AC-1/AC-6 sketch) currently writes `config.grey  # #9CA3AF` as a literal — that literal must resolve through the token below before implementation, not ship as a hard-coded hex. | `docs/standards/design/tokens.md` ("No literal hex or px may appear in application or catalogue code — only `var(--token)`"); `docs/standards/design/data-viz.md` §heat ramp (`--color-heat-none` — "unmatched / free-text outside vocabulary") |
| D-3 | Domain-colouring's palette-cycle-on-overflow (AC-3) SHALL draw from the design system's discrete categorical series palette, not an ad-hoc colour list. | `docs/standards/design/data-viz.md` ("Generic categorical series use a small, ordered, discrete series palette") — the same source this brief's own Design Decisions table already names for `domain_palette` |
| D-4 | Overlay colour information SHALL always be paired with legend text — never colour-only meaning. This restates the brief's own AC-7; cited here for design-system traceability. | This brief's AC-7; `docs/standards/design/components.md` component definition-of-done ("Meaning never colour-alone") |

### Gaps

- None beyond D-2 (the hex-literal note is itself the finding, not a missing citation).
