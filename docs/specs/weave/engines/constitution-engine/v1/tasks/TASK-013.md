---
type: Task
title: "Task: TASK-013 — Conversational Document Ingest Agent (E12-S1, USER PRIORITY)"
description: "The epic's Must story: upload a BPM/policy/runbook document and review
  ...
  HITL, committed via the TASK-012 spine (FR-038)."
...
created: 2026-07-08
blocked_by: [TASK-012]
unlocks: [TASK-014, TASK-016, TASK-019]
...
what TASK-014's chunker reuses)
...
The document extractor plugin (TASK-012 `Extractor` Protocol, kind=`document`) + the chat-panel
...
path. OUT: embeddings/corpus retrieval (TASK-014), vision (TASK-016), the import page
(TASK-019 — chat is this task's surface).

## Acceptance Criteria

| ID | Criterion (EARS) |
|---|---|
| AC-002-01 | WHEN the agent processes an uploaded document THE SYSTEM SHALL extract candidate entities + relationships, map them to BPMO kinds fetched from `GET /api/ontology/types` (authoritative — no hard-coded kind list), and emit proposals into the TASK-012 proposal store. |
...
| AC-002-05 | WHEN the human accepts/rejects in chat THE SYSTEM SHALL route through TASK-012's accept/reject endpoints — per proposal, no bulk auto-accept — with PROV-O naming the LLM as extracting agent and the human as approver, source doc as `prov:used`. |
...
```text
class DocumentExtractor(Extractor):                    # plugs into TASK-012 worker
    def extract(job):
        ...
        for cand in sonnet.call(prompt, tool=propose_entities_tool):   # typed tool output
            ...
            yield Candidate(ops=to_ops(cand),          # same Op shape as CE-WRITE-1
                            ...
                            source_span=cand.span)     # locator for TASK-014 citations
...
chat panel (frontend):
    ...
    accept/reject buttons -> TASK-012 endpoints; 422 renders violations on the card
```

## API Contracts

Consumes TASK-012's ingest endpoints (v1-delta §2) and **CE-READ-1** `GET /api/ontology/types`
(contracts.md — authoritative kind list). Mutations exclusively via TASK-012 accept →
...
| Splitter/chunking pins | [ADR-011](../../decisions/ADR-011.md) §1/1a | Parse scope guard shared with TASK-014 |
...
| Chat panel is the E12-S1 review surface (no new page) | Story text says "through the chat panel"; import page (TASK-019) serves the structured stories | engine spec E12-S1 |
...
- **blocked_by**: TASK-012 (spine: upload, proposal store, accept path)
- **unlocks**: TASK-014 (chunker reuses this parse), TASK-016 (vision variant of this flow),
  TASK-019 (E2E rides this surface)
...
- [x] Proposal flow + endpoints exist (TASK-012)
...
- `parse_simple`: markdown/docx via existing libs; PDF via text extraction only (pin 1a — no
  layout model). Keep the heading tree because TASK-014's chunker consumes it.
- Pitfall: candidate `source_span` locators must survive into the proposal row now — TASK-014's
