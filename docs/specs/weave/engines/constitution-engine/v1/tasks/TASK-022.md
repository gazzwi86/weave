---
type: Task Brief
title: "Task: TASK-022 — Versions Panel + Diff Overlay (CE-VERSION-1 / CE-DIFF-1)"
...
# Task: TASK-022 — Versions Panel + Diff Overlay

## Story

**Epic:** [EPIC-020](../../../constitution-engine.md#epic-020--version-views--diff--m2) (+ E4-S2 of
[EPIC-016](../../../constitution-engine.md#epic-016--visual-overlays--m2))
...
Covers: FR-016, FR-031, FR-032 ([constitution-engine.md §6.1](../../../constitution-engine.md#61-functional-requirements)).
Export is **JSON only in M2** (OQ-06 closed — m2-delta-explorer.md §1).
...
# Diff (Overlay registered in TASK-021 engine, exclusiveGroup "colour")
...
Errors: 401 unauthorised · 503 store unavailable. p95 ≤ 200 ms (m2-delta-explorer.md §4).
...
|---------|------|---------|---------|
| Component delta | `../../tech-spec/m2-delta-explorer.md` | §6 | Versions Panel + Diff/Version proxy edges |
| Sequence | `../../tech-spec/business-process-explorer.md` | version/graph-load flow | M1 paginated load the version reload reuses |

### Design Decisions

| Decision | Reference | Impact |
|----------|-----------|--------|
| Diff is server-computed (CE-DIFF-1); GE only renders | constitution-engine.md §6.5 | No client-side diffing of two loaded graphs — ever |
...
| OQ-06: JSON-only export in M2 | m2-delta-explorer.md §1 | No PDF/CSV code paths; invariants-explorer.md greps for their absence |
| Diff ⟷ heatmap mutual exclusion | FR-015/FR-016 | Register via TASK-021 engine `exclusiveGroup: "colour"` — no bespoke lockout |
...
- **blocked_by:** [TASK-021 (Overlay Engine — diff overlay registers into it)]
- **unlocks:** TASK-030 (release-gate suite covers version/diff paths)
...
- [ ] Conventional commit(s); PR references this task and EPIC-020
...
- The read-only canvas mode flag is the SAME mechanism ge-canvas-1.md rule 4 needs (version ⇒
  forced readonly). Build it as one context value; TASK-029 will consume it — do not duplicate.
- Removed elements aren't in the loaded draft graph: render them as ghost elements added by the
  ...
  via the TASK-021 engine handles colour, but ghost element cleanup is the diff overlay's
  `remove()`.
