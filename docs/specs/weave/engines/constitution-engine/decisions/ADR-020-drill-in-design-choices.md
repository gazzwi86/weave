---
type: Decision
title: "ADR-020: Drill-in design choices — neighbour reuse, expanded-state tracking, config-literal defaults"
description: "Engine-local decisions made while implementing TASK-013 (domain focus, neighbour
...
resource: docs/specs/weave/engines/constitution-engine/decisions/ADR-020-drill-in-design-choices.md
...
# ADR-020: Drill-in design choices (TASK-013)

**Scope:** [Graph Explorer](../../constitution-engine.md) M1, TASK-013 (domain focus, neighbour
expand/collapse; impact traversal is OQ-09-gated and out of scope for this ADR).

## Status

**Accepted** — implemented across the `feature/CE-EPIC-014` commits for TASK-013.
...
kind/relationship set is served by `GET /api/ontology/types` (`CE-READ-1`), not hard-coded. TASK-013
...
codebase at TASK-013 time, and the task brief's own pseudocode already established the
...
same open item already flagged for `domainMembershipPredicate` in TASK-013's design-decisions table
...
- No second CE-READ-1 call is ever made for neighbour expansion — network cost stays bounded to the
  spotlight fetch already paid for AC-1/AC-2 (TASK-011).
