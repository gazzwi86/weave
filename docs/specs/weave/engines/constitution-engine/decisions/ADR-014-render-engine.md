---
type: Decision
title: "ADR-014: Graph Explorer render engine — Cytoscape.js + fcose (default, spike-gated)"
description: "Engine-local decision for the Explorer force-canvas renderer. Defaults to Cytoscape.js + fcose (prototype-proven), pending the TASK-009 10k-node benchmark; names sigma.js/G6 (WebGL) as the contingency and estimates the renderer-swap rework delta on TASK-010..005 (council ENG-3). Disambiguates GE-local OQ-01/OQ-05 from program OQ-01."
tags: [decision, adr, graph-explorer, rendering, cytoscape, webgl, spike, m1]
status: Accepted
timestamp: 2026-07-01T00:00:00Z
resource: docs/specs/weave/engines/constitution-engine/decisions/ADR-014-render-engine.md
...
# ADR-014: Graph Explorer render engine — Cytoscape.js + fcose (default, spike-gated)

**Scope:** [Graph Explorer](../../constitution-engine.md) only. Output of the
[TASK-009 benchmark spike](../m1/tasks/TASK-009.md). Clears council **ENG-3** (renderer-swap
...
accepted deliberately). CE-TASK-012 is schema-unblocked.

## TASK-009 benchmark evidence (2026-07-05, updated with real prototype params)
...
1. **fcose params provenance chain** — `prototype-findings.md` does not exist in this repo; real
   ...
   See `.claude/state/escalations/TASK-009-blocker.md`.
...
or G6) per the "Decision" section below, and treat TASK-010 AC-7 as suspended until that renderer
...
[graph-explorer §2.2](../../constitution-engine.md#62-non-functional-requirements)). Cytoscape.js +
...
- **M1 force canvas is bounded to ≈1–2k visible nodes** (server-side pagination / level-of-detail /
  ...
  **retired for M1**. TASK-010's performance AC (AC-8) is rewritten against the bounded-canvas
  target, not the 10k target — flagged for the TASK-010 brief before it is implemented.
...
- The adapter-boundary invariant (next section) stays **mandatory** precisely because the WebGL swap
  is now a planned pre-v1 event, not a contingency — TASK-010..005 must build against the adapter so
...
**Isolation of the renderer (why the swap is survivable):** TASK-010..005 must treat the
...
Assumes the adapter-boundary invariant above holds. Delta = *additional* rework if TASK-009 is a
no-go and the renderer swaps to sigma.js/G6.

| Task | Feature | Renderer coupling | Rework delta on swap |
|------|---------|-------------------|----------------------|
| TASK-010 | Force canvas + navigation | **High** — layout, pan/zoom, drag are renderer-native | **~40–60%** re-implement against WebGL API (fcose→sigma force / G6 layout); the adapter interface itself is reusable |
| TASK-011 | Node spotlight + search overlay | **Low** — overlay is DOM/React above the canvas; needs node→screen-coords projection from the adapter | **~10–15%** (re-wire coordinate projection) |
| TASK-012 | Server-side layout persistence | **None** — persists `(x,y)` positions; renderer-agnostic (Aurora schema unchanged) | **~0–5%** (position-read hook only) |
| TASK-013 | Drill-in / expand-collapse / impact traversal | **Medium** — show/hide + sub-layout are renderer calls behind the adapter | **~20–30%** (re-map traversal → WebGL show/hide + re-layout) |

**Blended:** with the adapter boundary, a full no-go swap is a **~25–35%** rework of the
Explorer M1 canvas tasks, concentrated in TASK-010 — **not** a rewrite. Without the adapter
...
pre-estimated contingency, not an open risk; features TASK-011/012/013 are largely renderer-
agnostic by construction.

**Negative:** the adapter boundary adds a thin indirection layer (small complexity cost, well
inside Law E); the real 10k verdict is deferred to TASK-009 — until signed, TASK-010 AC-7
