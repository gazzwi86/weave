...
resource: docs/specs/weave/engines/constitution-engine/tech-spec/business-process-explorer.md
...
**Tenant isolation:** [ADR-014 — named-graph + query-rewriting](../../../decisions/ADR-001-tenant-isolation.md)
**Renderer strategy:** [ADR-014 (GE-local) — Cytoscape.js + fcose](../decisions/ADR-014-render-engine.md)
...
> swap to ~25–35 % rework. See [GE ADR-014](../decisions/ADR-014-render-engine.md).
...
TASK-009 runs a benchmark spike on a 10k-node dataset to determine whether Cytoscape.js + fcose
meets the M1 performance targets. This state diagram shows the go/no-go gate and its consequences.

```mermaid
stateDiagram-v2
    [*] --> Spike : TASK-009 begins
    ...
    CytoscapeConfirmed --> ADRAccepted : GE ADR-014 → Accepted (cytoscape+fcose)
    ADRAccepted --> [*] : TASK-010 AC-7 now assertable

    WebGLEscape --> RendererSwapADR : new ADR filed (sigma.js or G6)
    RendererSwapADR --> [*] : GE ADR-014 → Superseded<br/>TASK-010 AC-7 remains suspended
```

> **TASK-010 AC-7 is suspended until TASK-009 is signed off** (per GE ADR-014 Consequences).
> The engineer must not assert AC-7 until the benchmark completes. If WebGLEscape fires, the
> renderer adapter interface absorbs ~40–60 % rework in TASK-010 and ~25–35 % blended.
...
flowchart TD
    ...
    IT1 -->|No — blocked| IT2[/TASK-013 AC-6 + AC-7 suspended/]
...
This flow is gated on OQ-09 resolution (TASK-013 AC-6 and AC-7 suspended until then):
...
stateDiagram-v2
    ...
    note right of OverlayLoading
        TASK-013 AC-6 and AC-7 blocked
