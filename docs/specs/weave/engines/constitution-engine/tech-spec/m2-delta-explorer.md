...
resource: docs/specs/weave/engines/constitution-engine/tech-spec/m2-delta-explorer.md
...
**Scope rule:** this document contains ONLY changes from M1. `architecture-explorer.md`,
`data-model-explorer.md`, `business-process-explorer.md`, `testing-strategy-explorer.md` remain authoritative for
everything not restated here. Contract shapes stay canonical in
[`contracts.md`](../../../contracts.md) — this delta cites, never redefines.
Decisions: [ADR-018](../decisions/ADR-018-impact-traversal-predicate-closure.md) (predicate
closure), [ADR-019](../decisions/ADR-019-edit-attribution-principal-iri.md) (edit attribution),
[ADR-021](../decisions/ADR-021-m2-concurrency-client-drift-guard.md) (M2 concurrency = client
...
| OQ | Resolution |
|---|---|
| OQ-09 | 13-entry directed closure, shared config, drift guard — ADR-018. Unblocks M1 TASK-013 AC-6/AC-7 and M2 E4-S3. |
| OQ-11 | `actor` = JWT `principal_iri` claim (PLAT-IDENTITY-1, minted at first login by PLAT-TASK-004 AC-1) — ADR-019. E5-S3's "Cognito identity" wording superseded in letter, satisfied in substance. |
...
## 2. Aurora delta (extends data-model-explorer.md — same RLS pattern as `explorer_layout_positions`)
...
|---|---|---|
| `POST /api/proxy/operations/apply` | CE-WRITE-1 | Injects `actor` from JWT `principal_iri` claim (ADR-019); missing claim ⇒ reject loud. All E5 + GE-CANVAS-1 write-back traffic. |
...
Renderer-adapter invariant (ADR-014) applies unchanged: every new component drives the canvas
through the adapter interface; overlays are adapter operations, not direct renderer calls.

## 7. Testing delta (extends testing-strategy-explorer.md)

- **Concurrency (ADR-021):** E5-S3 GE-side drift guard — two-writer test asserting the second
...
- **Closure drift guard:** boot test with a stub `/api/ontology/types` missing one closure
  predicate ⇒ loud config error, traversal disabled (ADR-018).
...
Folded into [`invariants-explorer.md`](invariants-explorer.md) (M1 + M2, flat verify-by checklist — Arch Law 10).
