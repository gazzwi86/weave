...
TASK-012 defines it — the FastAPI layout-persistence service and its Aurora RLS. Every 10k
performance number is **gated on the OQ-01 spike (TASK-009)** and is not asserted as settled until
that spike signs off (architecture-explorer.md §10k-Node Performance Budget).
...
│   └── sigma.adapter.test.ts        # binds the SAME suite to the escape-hatch impl (ADR-014)
├── canvas/__tests__/
...
**Renderer-adapter contract suite (the crown-jewel test).** The ADR-014 swap is only bounded to
...
### Python (Layout Persistence Service — TASK-012)
...
async def test_save_rejects_mismatched_tenant_id(client: AsyncClient, jwt_tenant_a: str):
    # body tenant_id disagrees with the JWT claim → 403, nothing written (TASK-012 AC-6)
...
These six gates are cross-cutting invariants (architecture-explorer.md §Invariants); each maps to a named
...
|------|---------------|-------|-----------|
| 10k perf budget (TASK-010 AC-8, OQ-01) | WHEN the OQ-01 spike passes THEN first interactive render SHALL be ≤ 3 s @ 1k / ≤ 8 s @ 10k (p95) | Perf | `test_canvas_load_performance_p95_1k` / `_10k` |
| Cross-tenant layout RLS (TASK-012 AC-7) | WHEN a layout GET runs under a tenant-A JWT THEN zero tenant-B rows SHALL return, enforced by RLS at the DB layer | Integration | `test_cross_tenant_layout_isolation_rls` |
| Cross-tenant graph read (TASK-010 AC-9) | WHEN a graph load runs under a tenant-A JWT THEN zero tenant-B nodes or edges SHALL return | Integration | `test_cross_tenant_graph_load_isolation` |
| No partial render on CE error (TASK-010 AC-2) | WHEN CE-READ-1 errors or times out THEN the empty-state with retry SHALL show AND no node SHALL render | Integration | `test_canvas_shows_empty_state_on_ce_error` |
| Predicate closure config-loaded (TASK-013 AC-6) | WHEN impact traversal composes a query THEN the predicate path SHALL come from config, with no literal predicate IRI in code | Unit | `should NOT contain any hard-coded predicate IRI string literal` |
| IRI non-disclosure (TASK-011 AC-2) | WHEN a side panel renders for a non-ontologist THEN the raw IRI SHALL NOT be passed to the panel | Unit | `should NOT include raw IRI in side panel props when JWT role is "viewer"` |
...
The RDF store is **never** faked directly — the Explorer never touches it (architecture-explorer.md D1). All
...
|-------|---------------|-----------|--------|
| TASK-012 AC-3 | WHEN a viewer drags a node and reloads THEN THE SYSTEM SHALL restore it at the dragged position | `layout-persist.spec.ts` | Planned |
| TASK-011 AC-2 | WHEN a viewer spotlights a node THEN THE SYSTEM SHALL show label + type with no raw IRI visible | `spotlight.spec.ts` | Planned |
| TASK-010 AC-2 | WHEN CE-READ-1 errors during load THEN THE SYSTEM SHALL render the empty-state with retry, no nodes | `ce-error.spec.ts` | Planned |
| TASK-013 AC-1 | WHEN a viewer focuses a domain THEN only that domain's members SHALL stay at full opacity | `drill-in.spec.ts` | Planned |
| TASK-010 AC-9 | WHEN a tenant-A user loads the graph THEN zero tenant-B nodes SHALL render | `isolation.spec.ts` | Planned |
...
OQ-01-confirmed budget (≤ 3 s @ 1k) as a Playwright timing check — **suspended until TASK-009 signs
off** (TASK-010 AC-8). Accessibility: axe-core runs inside the E2E suite on the **non-canvas** UI
(side panel, search overlay, filter sidebar, comments) — **zero violations is a release gate**
(constitution-engine.md §Accessibility). The force canvas is exempt from full SR-navigation in v1 but is
...
(TASK-009 STEP 1). Each row matches the CE-READ-1 SPARQL SELECT shape, so the same fixtures drive
...
number below is a settled capability** until the OQ-01 benchmark spike (TASK-009) signs off — it is
the explicit go/no-go on the whole renderer choice (architecture-explorer.md §10k-Node Performance Budget).

### Canvas render budget (OQ-01 harness — gates the renderer)

| Budget | Target (p95, default, tunable) | Gate |
|--------|-------------------------------|------|
| Canvas initial load @ 1k nodes | ≤ 3 s to first interactive render | TASK-009 spike |
| Canvas initial load @ 10k nodes | ≤ 8 s to first interactive render | TASK-009 spike (go/no-go) |
| Node drag @ ≤ 1,000 visible nodes | ≥ 60 fps (≤ 16 ms/frame) | TASK-009 spike |
| Filter / overlay apply @ up to 10k | ≤ 300 ms | M2 |

Reference hardware: desktop Chrome latest, 16 GB RAM, **no GPU acceleration** (TASK-009 AC-1) — the
