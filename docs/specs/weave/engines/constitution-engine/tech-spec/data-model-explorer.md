...
tags: [graph-explorer, arch, tech-spec, m1, data-model]
status: Draft
timestamp: 2026-07-01T00:00:00Z
resource: docs/specs/weave/engines/constitution-engine/tech-spec/data-model-explorer.md
...
> through the Constitution Engine (CE) via the named-graph query-rewriter (ADR-014). GE's ONLY
...
Per [GE ADR-014](../decisions/ADR-014-render-engine.md), every task in the M1 Explorer must treat
...
TASK-010..005. It is a hard architectural invariant — calling renderer APIs directly is rejected
at review.
...
The canonical named-graph scheme (from [ADR-014](../../../decisions/ADR-001-tenant-isolation.md)):
...
> the closure for impact traversal* (TASK-013 AC-6 / AC-7) is **not hard-coded** in GE. The
> predicate set will be confirmed by the CE team at OQ-09 resolution and consumed via
> `config.oq09_predicate_closure`. TASK-013 AC-6 and AC-7 are **blocked** until OQ-09 is closed.
...
The DDL below is normative (TASK-009 AC-4 mandates this exact schema):
