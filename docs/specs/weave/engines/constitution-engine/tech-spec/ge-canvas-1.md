...
resource: docs/specs/weave/engines/constitution-engine/tech-spec/ge-canvas-1.md
...
this pin). **Provider stories:** E9-S1 (constitution-engine.md).
...
and all — ADR-014-render-engine invariants apply unchanged inside the embedded instance).
...
| 5 | `readonly:false` edits commit ONLY through CE-WRITE-1 with ADR-019 principal-IRI attribution; SHACL `422` and timeout-rollback behaviour identical to the full Explorer (FR-019–022) | `should write back through CE-WRITE-1 when edited` |
...
(constitution-engine.md §Roadmap M2) and the Build-M2 unblock evidence for the
...
flowchart LR
    subgraph spa[Weave SPA monorepo]
        ...
        subgraph ge[Graph Explorer package]
            GC[GraphCanvas component] --> AD[Renderer adapter ADR-014]
        end
