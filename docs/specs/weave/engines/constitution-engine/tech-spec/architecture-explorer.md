...
strict) — not a separate application (constitution-engine.md §Constraints). It renders the company graph
...
table (data-model-explorer.md §Layout Schema). Per weave-spec §1.2 the Explorer M1 is delivery **wave W2**,
...
benchmark spike (TASK-009) fails its 10k-node go/no-go (ADR-014-render-engine). No 10k performance
...
C4Context
    ...
    System_Ext(cognito, "AWS Cognito", "Human identity provider; JWT with tenant_id + role + principal_iri claims (no workspace_id claim — ADR-017)")
...
C4Container
    ...
    Container_Boundary(explorer, "Graph Explorer") {
        Container(canvas, "SPA Canvas Module", "Next.js 15 / TS strict / Cytoscape.js + fcose", "Force canvas, spotlight, search, drill-in; renderer behind an adapter (sigma.js/G6 WebGL escape hatch, ADR-014)")
    ...
    Container_Boundary(aws, "AWS") {
        ...
        Container(cognito, "AWS Cognito", "AWS Cognito", "JWT issuance; tenant_id + role + principal_iri claims (no workspace_id claim — ADR-017)")
    }
...
- **Layout Persistence Service** is the only component that writes anything, and only to the
  GE-owned Aurora table. It sets the RLS GUC inside every transaction before any DML (TASK-012).
...
C4Component
    ...
    Container_Boundary(canvas, "SPA Canvas Module") {
        ...
        Component(adapter, "Renderer Adapter", "TS interface", "load / onNodeClick / getViewport / setLayout / pin (data-model-explorer.md); hard boundary — direct renderer calls rejected at review")
...
WebGL engine) directly, which is what bounds the ADR-014 swap to a ~25-35% delta. And every
...
(TASK-009) signs off — it is the explicit go/no-go gate on the whole renderer choice.
...
(TASK-009 AC-1) — the conservative floor, so a pass generalises upward.

**Go/no-go (ADR-014-render-engine).** The default is Cytoscape.js + fcose (prototype-proven at
smaller scale). TASK-009 STEP 4 flips the ADR to `Accepted` on a go (render ≤ 8 s @ 10k, drag
...
[`../../../decisions/`](../../../decisions/) (ADR-014 tenant isolation); the renderer decision is
[`../decisions/ADR-014-render-engine.md`](../decisions/ADR-014-render-engine.md).
...
- **Tenant-scoped layout reads/writes:** WHEN any `/api/layout/positions` read or write is processed
  ...
  (TASK-012 AC-7 release-gate isolation test).
...
- **No partial render on CE error:** WHEN a CE-READ-1 call errors or times out (default 10 s,
  ...
  control AND SHALL NOT display any partially loaded graph (FR-001, TASK-010).
...
- **Renderer contract stability:** WHEN the render engine is swapped (Cytoscape → WebGL) THE SYSTEM
  ...
  feature component calls a renderer API directly (ADR-014-render-engine; rejected at review).
- **Layout durability:** WHEN a layout save fails THE SYSTEM SHALL hold the position optimistically
  ...
  SHALL NEVER be silently dropped (TASK-012 AC-2).
- **IRI non-disclosure:** WHEN a side panel renders for a non-ontologist principal THE SYSTEM SHALL
  ...
  panel SHALL show a generic not-found state (data-model-explorer.md §Node Properties).
- **Accessibility:** WHEN any non-canvas Explorer UI (side panel, search overlay, filter sidebar,
  ...
  SHALL NOT trap keyboard focus (constitution-engine.md §Accessibility).
...
Targets are the constitution-engine.md §6.2 configurable defaults (provisional, not contractual SLAs);
...
| Canvas initial load @ 10k | ≤ 8 s p95 — **unverified, OQ-01 spike gate** | OQ-01 benchmark report (TASK-009) | Renderer bet fails; triggers WebGL escape hatch (OQ-05) |
