...
[m2-delta.md](../../tech-spec/m2-delta.md) §2–§3 · Owning surfaces: GE M2 TASK-001 (filters),
TASK-002 (overlay engine), TASK-003 (versions + diff); CE M2 TASK-006 (rules & policies screen).
...
- **unlocks**: TASK-005
- **External (DoR, not DAG):** GE M2 TASK-001/002/003 merged (filters, overlays, versions);
  CE M2 TASK-006 merged (rules screen). Anchors stay `shipped: false` until planted here, so
...
- [ ] GE M2 TASK-001/002/003 and CE M2 TASK-006 merged, or tours explicitly left unshipped
      (`shipped: false`)
