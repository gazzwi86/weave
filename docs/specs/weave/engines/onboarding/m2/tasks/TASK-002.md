...
[m2-delta.md](../../tech-spec/m2-delta.md) §2–§3 · Owning surfaces: GE M2 TASK-002 (overlay
panel) + TASK-008 (completeness overlay), Platform M2 TASK-017 (role-home completeness tile).
...
| AC-002-03 | WHEN the role-home completeness tile renders THE SYSTEM SHALL show a first-visit beacon on `plat.role-home.completeness-map` linking "See gaps in Explorer" — a plain deep-link to the Explorer route (GE M2 TASK-008 exposes no overlay-on URL state); the `ge.overlay.controls` beacon (AC-002-02) picks the user up there, and the beacon copy SHALL say the overlay is enabled from the Layers panel. |
...
- **unlocks**: TASK-005
- **External (DoR, not DAG):** GE M2 TASK-002/008 merged (overlay panel + completeness overlay
  exist to plant on); Platform M2 TASK-017 merged (tile exists — its anchor is planted by
...
- [ ] GE M2 TASK-002/008 + Platform TASK-017 merged, or explicitly deferred with anchors left
      `shipped: false`
