...
| Artefact generation | Next.js + FastAPI from approved spec; safety gates before commit; deploy + demo |
...
| Technical architect | Reviews tech spec; approves phase-gate ceremony; manages project settings |
...
- Tech stack: Next.js 15 + FastAPI + AWS Cognito + Bedrock; Python `uv` only for tooling.
...
| FR-002 | AI spec drafting: brief/PRD/tech-spec streamed via `CE-READ-1` (pinned version); 60 s timeout, tunable; model `claude-opus-4-8` | E1-S1 | Must | M1 |
...
| FR-010 | Integration binding by reference via `PLAT-CONNECTOR-1`; no connector credential stored in Build | E2-S3 | Must | v1.0 |
...
| FR-043 | Dependency-summary handoff: CODIFY writes key decisions + edge cases to tenant-scoped store; PLAN reads predecessors' summaries before DELEGATE; missing summary = hold in Ready | E11-S3 | Must | M1 |
...
| FR-055 | Pre-scaffold spec-review gate: cascade check (brief→PRD→roadmap→tech-spec→impl-ready) with hard blockers per transition; critical gap halts scaffolding | E12-S6 | Must | M1 |
...
> pinned CE version to scope its state spine (FR-044). A **minimal backend project record**
> (IRI + pinned version, no Registry UI) is an M1 prerequisite. The full Project Registry grid,
> settings tabs, and budget cascade UI (FR-006 through FR-012) land in v1.0.
...
members/roles, integrations, secrets, data classification, pinned ontology version).

**Stories:** E2-S1 (grid), E2-S2 (auto-create), E2-S3 (governance cascade) — all Must Have.
...
**Milestone:** v1.0 · **Priority:** Must Have (S1) / Should Have (S2) · **Status:** Backlog
...
generation pipeline reads spec + BPMO project graph via `CE-READ-1` and produces OpenAPI →
FastAPI routes → Next.js pages/components. Every gate is distinct and atomic.
...
**Milestone:** M1 (S1–S5); M2 (S6 preflight/scaffolding/self-verification/investigator); post-v1 (S6 durable memory) ·
**Priority:** Must Have (S1–S5) / Should Have (S6 M2 parts) · **Status:** Backlog
**depends_on:** E6 (run-mode + retry contract), E5 (typed brief), E2 (project record), `PLAT-SETTINGS-1`, `PLAT-BILLING-1`, `PLAT-NOTIFY-1`
**blocks:** E4, E3, E12 · **provides:** run lifecycle

Epic 11 owns the loop mechanics that wrap E6's run-mode and governance contracts: the bounded
autonomous loop with a hard turn cap, the PLAN→DELEGATE→ASSESS→CODIFY lifecycle, the
dependency-summary handoff, the tenant-scoped RLS state spine, and configurable model routing.

**Partition rule (with E6):** E6 owns run modes, retry taxonomy, and HITL gates. E11 owns the
loop mechanics, per-task lifecycle, dep-summary handoff, state spine, and model routing.

**Stories:** E11-S1 (turn cap + resumability), E11-S2 (PDAC lifecycle), E11-S3 (dep summary
handoff), E11-S4 (state spine), E11-S5 (model routing) — all M1 Must Have. E11-S6 (preflight,
scaffolding gate, self-verification, investigator) — M2 Should Have; durable memory — post-v1.
...
flowchart LR
    ...
    PRE_SCAFFOLD --> DoR["E12-S1\nDoR Gate\n(in PLAN)"]
    DoR --> GEN["E8-S1 App Generation\n(OpenAPI → FastAPI → Next.js)"]
...
|---|---|---|
| **M1 — Thin Proof** | Model → generate → write-back loop on Hammerbarn pre-seed demo | E1 Request Studio · E6 Spec Lifecycle + Run Modes · E11-S1–S5 Dark-Factory core · E8-S1 App generation + S4 Deploy/demo · E9-S1 Write-back · E12-S1 DoR + S2 DoD + S6 Pre-scaffold; task-brief schema (FR-018) |
| **M2 — Legibility + Trust** | Brand conformance; SDK; quality ceremonies; orchestrator hardening | E8-S3 Anatomy/wiki · E8-S5 BE-SDK-1 (incl. CE-FUNCTION-1 bindings) · E8-S1 CE-BRAND-1 gate · E11-S6 preflight/scaffolding/self-verify/investigator · E12-S3 Full QA · E12-S4 Coverage audit · E12-S5 Phase-gate ceremony · E9-S2 Staleness · E8-S4 Release plan |
| **v1.0 — Lighthouse** | Full PM surfaces for concurrent production projects | E2 Project Registry + settings · E3 Project Dashboard · E4 Kanban · E5 Task Brief/Detail UI (5-tab panel) · E7 Decision Log (E12 fully covered M1+M2; S7/S8 post-v1) |
...
1. Hammerbarn pre-seed: a product owner completes intake (E1), the dark factory runs and generates
   a working Next.js + FastAPI application, and all M1 safety gates pass.
