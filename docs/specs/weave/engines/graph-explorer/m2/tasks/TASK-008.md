---
type: Task Brief
title: "Task: TASK-008 — Model-Completeness Map (coverage_gap overlay)"
...
# Task: TASK-008 — Model-Completeness Map

## Story

**Epic:** [EPIC-010](../../../graph-explorer.md#epic-010--model-completeness-map--m2-new)
...
Covers: FR-035, FR-036 ([graph-explorer.md §2.1](../../../graph-explorer.md#21-functional-requirements)).
...
```
# Registered as a TASK-002 overlay — but in the BADGE channel, not exclusiveGroup "colour".
completenessOverlay = {
...
|---------|------|---------|---------|
| Component delta | `../../tech-spec/m2-delta.md` | §6 | Completeness Overlay → CE-Read proxy path |
| C4 L3 | `../../tech-spec/architecture.md` | Component — SPA Canvas Module | Spotlight/side-panel components the drill extends |

### Design Decisions

| Decision | Reference | Impact |
|----------|-----------|--------|
| Gap indicator = badge, not colour | m2-delta.md §1 (OQ-08 deferred) | Coexists with domain colouring; only diff blocks it (diff owns ghost elements) |
...
| Missing-link names humanised from CE types labels | M1 IRI-hiding rule | `humaniseRelName` uses the boot-time types palette; no literal predicate strings (invariants.md grep) |
| Inline shortcut reuses TASK-004 edge-draw with preset predicate | FR-036 | Soft dependency: feature-detect the edit controller; CE-surface link is the fallback, so TASK-004 absence never blocks this task |
...
- **blocked_by:** [TASK-002 (overlay engine + legend + badge channel)]
- **unlocks:** TASK-011
- **Soft:** TASK-004/005 for the inline edit shortcut (feature-detected; CE-surface link
...
- [ ] Conventional commit(s); PR references this task and EPIC-010
...
- The badge is an adapter capability — if `setBadge`/`clearBadges` don't exist yet, extend the
  adapter interface (ADR-001 route), never decorate Cytoscape elements directly from the
  overlay.
