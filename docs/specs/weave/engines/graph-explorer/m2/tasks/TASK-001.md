---
type: Task Brief
title: "Task: TASK-001 — Filters & Layers Panel (entity, relationship, property, governed-content)"
...
# Task: TASK-001 — Filters & Layers Panel

## Story

**Epic:** [EPIC-003](../../../graph-explorer.md#epic-003--filters--layers--m2)
...
Covers: FR-011, FR-012, FR-013, FR-014 ([graph-explorer.md §2.1](../../../graph-explorer.md#21-functional-requirements)).
...
```
# All canvas mutations go through the renderer adapter (ADR-001) — never cytoscape directly.
...
|---------|------|---------|---------|
| Component delta | `../../tech-spec/m2-delta.md` | §6 | Filter Panel position and its adapter/proxy edges |
| C4 L3 | `../../tech-spec/architecture.md` | Component — SPA Canvas Module | Adapter boundary the panel must drive |

### Design Decisions

| Decision | Reference | Impact |
|----------|-----------|--------|
| Property filter = client-side visual filtering only, never a CE query | graph-explorer.md §2.5 | `evalFilter` reads loaded element data; adding a fetch is a defect |
| Renderer adapter is the only canvas access path | ADR-001-render-engine | All hide/dim/highlight ops are adapter methods; extend the adapter if a needed op is missing (adapter change, not a direct call) |
| Shared panel shell built here | m2-delta.md §6 | TASK-002/003/007/008 mount into this shell; keep it presentation-only (no filter logic inside the shell) |
| Filter/overlay apply ≤ 300 ms @ 10k | m2-delta.md §4 | Batch adapter style updates (single `adapter.batch()` call), never per-node loops of individual style writes |
...
- **unlocks:** TASK-002 (overlay engine mounts in this panel shell and interacts with filter
  state), TASK-007 (views panel reuses the shell; saved views serialise this FilterState)
...
- [ ] FilterState is serialisable to JSON unchanged (TASK-007 saved-view dependency)
...
- `FilterState` must be a plain serialisable object — TASK-007 stores it verbatim in
...
- Governed-layer queries: cite CE-READ-1 via the existing proxy only; layer SPARQL lives beside
  the M1 query builders, predicates from config (never literal in component code — invariants.md).
