---
type: Task Brief
title: "Task: TASK-006 — Explorer Persistence Service: Saved Views + Comments (Aurora)"
...
blocked_by: []
unlocks: [TASK-007]
adr_refs: [ADR-004-layout-persistence-backend-conventions, ADR-006-edit-attribution-principal-iri]
...
# Task: TASK-006 — Explorer Persistence Service: Views + Comments

## Story

**Epic:** [EPIC-007](../../../graph-explorer.md#epic-007--saved-views--layout--m2) (backend;
plus E6-S1/S2 server side of [EPIC-006](../../../graph-explorer.md#epic-006--async-share--comments--m2))
...
Extends the M1 Layout Persistence Service (same FastAPI container — m2-delta.md §3); DDL is
normative in [m2-delta.md §2](../../tech-spec/m2-delta.md).
...
POST /api/comments (body: { target_kind, target_ref, body }):
  if "author" in body: return 400                   # AC-6 spoof guard (ADR-006 pattern)
...
|---------|------|---------|---------|
| Component delta | `../../tech-spec/m2-delta.md` | §6 | Persistence Service edges (Aurora, PLAT-NOTIFY-1) |
| ERD (M1) | `../../tech-spec/data-model.md` | Entity Relationship Diagram | Layout table the snapshot reuses |
| DDL (M2) | `../../tech-spec/m2-delta.md` | §2 | Normative table definitions this task migrates |
...
- **unlocks:** TASK-007 (all UI for these endpoints)
...
- Copy the M1 layout-service session/GUC helper verbatim (ADR-004) — do not reinvent the
