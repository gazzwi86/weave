...
| AC-8 | WHEN the M1 force canvas renders its **bounded visible-node set** (≈1–2k nodes; the whole-company 10k-at-once target was retired for M1 per ADR-001, human sign-off 2026-07-05), THE SYSTEM SHALL complete first interactive render within ≤ 3 s at ≤ 1k visible nodes (p95, tunable) on reference hardware (desktop Chrome latest), and beyond the visible bound SHALL rely on server-side pagination / level-of-detail / domain-focus drill-in rather than rendering all nodes at once. The retired 10k-single-canvas threshold moves to the **pre-v1 WebGL migration** (ADR-001 preferred renderer), not M1. | `test_canvas_load_performance_p95_1k`, `test_canvas_bounded_visible_node_set` |
...
|----------|-----------|---------------------|
| Cytoscape.js + fcose for the M1 bounded force canvas (TASK-001 signed off 2026-07-05: 10k-at-once retired for M1, WebGL preferred pre-v1) | [ADR-001](../../decisions/ADR-001-render-engine.md) | Use fcose with prototype params verbatim; build against the render-adapter boundary so the pre-v1 WebGL swap is render-layer-only; AC-8 targets the bounded ≤1k-visible render, not 10k |
