---
type: Decision
title: "ADR-002: TASK-002 AC-8 perf target rescoped to the M1 bounded visible-node budget"
description: "Real-component measurement (not the TASK-001 bespoke harness) confirms Cytoscape.js + fcose misses the 3000ms p95 target at 1k dense nodes. Human decision: rescope AC-8's M1 gate to the bounded visible-node budget fetchGraph already enforces; defer the dense 1k-node case to a later GE milestone."
tags: [decision, adr, graph-explorer, rendering, performance, m1]
status: Accepted
timestamp: 2026-07-05T00:00:00Z
resource: docs/specs/weave/engines/graph-explorer/decisions/ADR-002-ac8-perf-rescope.md
...
# ADR-002: TASK-002 AC-8 perf target rescoped to the M1 bounded visible-node budget

**Scope:** [Graph Explorer](../../graph-explorer.md) TASK-002 (Whole-Company Force Canvas +
Navigation), AC-8 (canvas load performance) only. Builds on [ADR-001](ADR-001-render-engine.md),
...
TASK-002's QA pass flagged AC-8 as unverified: no perf E2E spec existed. The Engineer built one
...
`ExplorerCanvas`/`useExplorerCanvas` stack — not the TASK-001 bespoke standalone harness.
...
All three runs miss the target by roughly 2x, consistent with (and worse than) the TASK-001 OQ-01
...
(≈1000-1400 nodes, checked at CE-READ-1 page boundaries; see ADR-001's "≈1-2k" framing). This
...
- **Deferred to:** a later GE milestone, gated on either the WebGL renderer swap already recorded
  as *preferred* in ADR-001 (sigma.js/G6), or a precomputed-layout path (server-side layout
  persistence, TASK-004, could seed initial positions and skip fcose's iterative convergence on
...
- **Optimize Cytoscape/fcose to hit 3000ms at 1k dense nodes within M1** — rejected: the residual
  gap (≈2x, and ≈1.8x on the OQ-01 spike before that) is not tunable-margin territory per ADR-001's
