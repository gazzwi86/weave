...
- **Natural-language authoring is the biggest advantage.** A user describes an automation in
  ...
  (or the referenced BPMN activity) in the ontology" — and an AI agent builds it, grounded in
  the exact process step it points at. A visual, n8n-style flow canvas complements this for
  inspection and manual alteration, and the two representations stay in sync.
- **Automations are portable agent artefacts.** Each automation is implemented as Anthropic
  Agent SDK material (skills, commands, agents), so it is portable, versioned, and reusable
  rather than locked inside a proprietary workflow tool.
...
- **Primary — integrated-system events:** webhooks, Jira, ServiceNow, Slack, cron/schedule,
  and similar events from connected systems. (Specific connector priority is deferred to the
  roadmap.)
...
- A conversational AI automation builder where the user describes an automation in plain
  language that references the model directly — specific entities, processes, or BPMN
  activities in the ontology — and an agent creates or edits it, grounded in what was
  referenced.
...
- Automations are implemented as Anthropic Agent SDK material (skills, commands, agents) —
  portable, versioned, and reusable, not locked inside a proprietary workflow tool.
...
- **Authoring the ontology, processes, rules, or BPMN models themselves** — that is the
  Constitution Engine; the Events Engine consumes and references them.
...
- [ ] **Natural-language authoring works and is grounded.** A user creates a working
      automation by describing it in plain language that references a specific ontology process
      or BPMN activity, with no manual coding, and the resulting automation is linked to the
      element it referenced. Measured by authoring telemetry and the automation-to-ontology
...
- [ ] **The two representations stay in sync.** An automation authored conversationally is
      ...
      GA.
- [ ] **Automations are portable agent artefacts.** Each automation is emitted as versioned
      Anthropic Agent SDK material (skill, command, or agent). Measured by inspecting the
      artefact store; source: the automation registry. Target: at GA.
...
- Automations are implemented as Anthropic Agent SDK material (skills, commands, agents),
  consistent with the Build Engine's agent SDK decision.
- Every automation must be grounded in the ontology — it references a process, rule, or BPMN
  activity; orphan automations with no link to the model are not allowed.
...
- The audit trail is append-only and tamper-evident; no agent can alter it.
- Connectors are a platform-managed capability the engine consumes; secrets and credentials
  are held in AWS Secrets Manager, never in automation definitions.

**Business**

- Usage-based revenue is tied to automation runs (per the platform pricing decision), so runs
  must be metered.
...
| Natural-language-first authoring, with the NL able to reference ontology processes and BPMN activities directly | This is the engine's biggest advantage — describe an automation grounded in a specific documented process, no code | 2026-06-26 |
...
| Automations are implemented as Anthropic Agent SDK material (skills, commands, agents) | Portable, versioned, reusable; consistent with the Build Engine's agent SDK choice | 2026-06-26 |
...
- **Triggers & connectors** — configured event sources and action targets (webhooks, Jira,
  ServiceNow, Slack, cron) and their health.
